...
# Globals
#
roach_bof       = 'testge_aj_2012_Oct_23_1937.bof'
tengbe_device   = 'gbe0'
payload_len     = 1026
period          = 1350
...
if (roach_id < 0 or roach_id >= num_roach):
  sys.stderr.write("WARNING: roach_id not in valid range\n")
...
roach_number = 0
Dada.logMsg(2, DL, "["+rid+"] pkt_sim_roach_number " + str(roach_number))
fpga.write_int('pkt_sim_roach_number', roach_number)
...
while ((not arp_valid) and attempts < 60):
  ...
  for line in lines:
    Dada.logMsg(3, DL, "["+rid+"] ARP [" + line + "] == '" + arp_line + "'")
    if (line == arp_line):
      arp_valid = False
