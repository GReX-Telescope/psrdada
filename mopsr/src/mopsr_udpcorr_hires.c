...
int main (int argc, char **argv)
{
  ...
  // handle the mis-mapping of antenna in the UDP packets
#ifdef HIRES
  fprintf (stderr, "Requested antennas (%d, %d)\n", ctx->ant1, ctx->ant2);
  ctx->ant1 = mopsr_get_hires_ant_number (ctx->ant1);
  ctx->ant2 = mopsr_get_hires_ant_number (ctx->ant2);
  fprintf(stderr, "Packed in UDP packets (%d, %d)\n", ctx->ant1, ctx->ant2);
#endif
