...
{
  fprintf (stdout,
    "mopsr_ibdb_SFT [options] recv_id cornerturn.cfg\n"
    " -k <key>          hexadecimal shared memory key  [default: %x]\n"
    " -s                single transfer only\n"
    " -v                verbose output\n"
    " -h                print this usage\n\n"
    " recv_id           beam block to receive data for\n"
...
int64_t mopsr_ibdb_recv (dada_client_t* client, void * buffer, uint64_t bytes)
{
  ...
  if (ascii_header_set (buffer, "NCHAN", "%u", new_nchan) < 0)
  {
 ...
 */
int mopsr_ibdb_ib_init (mopsr_bp_ib_t * ctx, dada_hdu_t * hdu, multilog_t * log)
{
  if (ctx->verbose > 1)
...
int mopsr_ibdb_destroy (mopsr_bp_ib_t * ctx) 
{
  unsigned i=0;
  int rval = 0;
  for (i=0; i<ctx->nconn; i++)
...
int mopsr_ibdb_open_connections (mopsr_bp_ib_t * ctx, multilog_t * log)
{
  dada_ib_cm_t ** ib_cms = ctx->ib_cms;
...
void * mopsr_ibdb_init_thread (void * arg)
{
  ...
  {
    multilog(log, LOG_ERR, "open_connections: dada_ib_accept failed\n");
    pthread_exit((void *) &(ib_cm->ib_connected));
  }
...
int main (int argc, char **argv)
{
  ...
  if (verbose)
    multilog (log, LOG_INFO, "main: mopsr_setup_bp_cornerturn_recv(%d)\n", recv_id);
  ctx.conn_info = mopsr_setup_bp_cornerturn_recv (cornerturn_cfg, &ctx, recv_id);
  if (!ctx.conn_info)
  {
