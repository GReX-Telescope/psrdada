...
int64_t mopsr_dbib_send (dada_client_t* client, void * buffer, uint64_t bytes)
{
  ...
  unsigned nbit;
  if (ascii_header_get (buffer, "NBIT", "%d", &nbit) != 1)
  {
  ...
  // ensure the NBEAM in the data block matches that in the cornerturn config
  if (old_nbeam != ctx->nbeam_send)
  {
    multilog (log, LOG_ERR, "send: NBEAM in datablock [%u] did not match "
              "cornerturn configuration [%u]\n", old_nbeam, ctx->nbeam_send);
    return -1;
  }

  // use the cornerturn configuration to determine this
  new_nbeam = ctx->nbeam_recv;
  ...
  // this is the factor by which the data rate is changed on the sending side only!
  unsigned divisor = old_nbeam / new_nbeam;
  if (ctx->verbose)
    multilog (log, LOG_INFO, "send: old_nbeam=%u, new_nbeam=%u divisor=%u\n", old_nbeam, new_nbeam, divisor);

  new_bytes_per_second = old_bytes_per_second / divisor;
  if (ctx->verbose)
  ...
  new_resolution = old_resolution / divisor;
  if (ctx->verbose)
  ...
  new_obs_offset = old_obs_offset / divisor;
  if (ctx->verbose)
  ...
  if (old_file_size > 0)
  {
    new_file_size = old_file_size / divisor;
    if (ctx->verbose)
  ...
  {
    if (ctx->verbose)
      multilog (log, LOG_INFO, "open: setting transfer size for iconn=%d\n",i);
                               ...
                               uint64_t bytes, uint64_t block_id)
{
  ...
  if (ctx->verbose)
    multilog (log, LOG_INFO, "send_block: bytes=%"PRIu64" nbeam_send=%u "
              "nchan_send=%u ndim=%u nbyte=%u\n", bytes, ctx->nbeam_send, ctx->nchan_send,
              ctx->ndim, ctx->nbyte);

  // get the number of time samples in the input data block
  const uint64_t nsamp = bytes / (ctx->nbeam_send * ctx->nchan_send * ctx->ndim * ctx->nbyte);

  const uint64_t chan_stride = nsamp * ctx->ndim * ctx->nbyte;

  // number of bytes to be copied in each rdma operation (1 beam, 4 chans, 1 dim, 4 bytes)
  const uint64_t rdma_bytes = ctx->nchan_send * chan_stride;

  // channel offset for this sender (same offset in all receivers)
  const uint64_t recv_chan_offset = ctx->conn_info[0].chan_first * chan_stride;

  // number of bytes for a recv's beam with all channels
  const uint64_t recv_beam_stride = ctx->nchan_recv * chan_stride;

  if (ctx->verbose)
    multilog (log, LOG_INFO, "send_block: bytes=%"PRIu64" nsamp=%"PRIu64" chan_stride=%"PRIu64
              " rdma_bytes=%"PRIu64" recv_chan_offset=%"PRIu64" recv_beam_stride=%"PRIu64"\n", 
              bytes, nsamp, chan_stride, rdma_bytes, recv_chan_offset, recv_beam_stride); 
  ...
  sge.length = rdma_bytes;

  send_wr.sg_list  = &sge;
  send_wr.num_sge  = 1;
  send_wr.next     = NULL;

  unsigned iconn, ibeam, obeam;
  mopsr_bp_conn_t * conn;

  for (iconn=0; iconn<ctx->nconn; iconn++)
  {

    for ( ibeam =  ctx->conn_info[iconn].beam_first; 
          ibeam <= ctx->conn_info[iconn].beam_last; 
          ibeam++ )
    {
      sge.lkey = ib_cms[iconn]->local_blocks[block_id].buf_lkey;

      obeam = ibeam - ctx->conn_info[iconn].beam_first;
      // increase remote channel memory address by relevant amount 
      send_wr.wr.rdma.remote_addr = remote_buf_va[iconn] + (obeam * recv_beam_stride) + recv_chan_offset; 
      send_wr.wr.rdma.rkey        = remote_buf_rkey[iconn];

      send_wr.opcode              = IBV_WR_RDMA_WRITE;
      send_wr.send_flags          = 0;
      send_wr.wr_id               = ibeam;

      //multilog(log, LOG_INFO, "iconn=%d ibeam=%d obeam=%d in_offset=%d out_offset=%d\n", iconn, ibeam, obeam, (sge.addr - (uintptr_t) buffer), (send_wr.wr.rdma.remote_addr - remote_buf_va[iconn]));

      if (ibv_post_send (ib_cms[iconn]->cm_id->qp, &send_wr, &bad_send_wr))
      {
        multilog(log, LOG_ERR, "send_block: ibv_post_send [iconn=%u, ibeam=%u] failed\n", iconn, ibeam);
        return -1;
      }

      // incremement local address pointers by 1 beams's stride
      sge.addr += rdma_bytes;
    }
  }
...
int mopsr_dbib_ib_init (mopsr_bp_ib_t * ctx, dada_hdu_t * hdu, multilog_t * log)
{
  ...
  const unsigned modulo_size = ctx->nchan_send * ctx->nbeam_send * ctx->ndim * ctx->nbyte;
  ...
  for (i=0; i < ctx->nconn; i++)
  {
    ...
    ctx->ib_cms[i]->send_depth = ctx->nbeam_recv + 1;
