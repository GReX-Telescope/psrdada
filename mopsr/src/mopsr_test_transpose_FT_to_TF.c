...
int main (int argc, char **argv)
{
  ...
  size_t nbytes = nchan * nsamp * ndim;
  ...
  unsigned npart = block_size / (nchan * ndim);
  unsigned ipart, ichan;
...

  {
    ...
    multilog (log, LOG_INFO, "mopsr_input_transpose_FT_to_TF (%d, %d)\n", block_size, nchan);
    mopsr_input_transpose_FT_to_TF (stream, d_in, d_out, block_size, nchan);
    check_error_stream( "mopsr_input_transpose_FT_to_TF", stream);
  ...
  for (ipart=0; ipart<npart; ipart++)
  {
    for (ichan=0; ichan<nchan; ichan++)
    {
      ...
      {
        if (nerr < 50)
          multilog (log, LOG_INFO, "[%d][%d] WRONG (%d, %d) != %d\n", ipart, ichan, h_out_ptr[0], h_out_ptr[1], val);
        nerr ++;
      }
      h_out_ptr += 2;
    }
  ...
  for (ichan=0; ichan<nchan; ichan++)
  {
    for (ipart=0; ipart<npart; ipart++)
    {
      h_in_ptr[0] = (int8_t) (ichan);
      h_in_ptr[1] = (int8_t) (ichan);
      h_in_ptr += 2;
    }
  }
