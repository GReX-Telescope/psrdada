...
void plot_cross_vs_freq (float * amps, float * phases, unsigned npts, char * device, float bw_per_pt, float base_freq);
...
void usage()
{
  fprintf (stdout,
     ...
     " -r              reset cross correlation each block\n"
...
int main (int argc, char **argv)
{
  ...
  while ((arg=getopt(argc,argv,"b:d:e:f:g:D:hl:n:p:rv")) != -1)
  {
    switch (arg)
    {
      case 'b':
  ...
  {
    fprintf(stderr, "could not read PFB_ID1 from header\n");
    return -1;
  }
  char pfb_id_str2[5];
  if (ascii_header_get (header, "PFB_ID2", "%s", pfb_id_str2) != 1)
  {
    fprintf(stderr, "could not read PFB_ID2 from header2\n");
    return -1;
  }
  ...
  {
    fprintf(stderr, "could not read ANT_ID1 from header\n");
    return -1;
  }
  int ant_id2;
  if (ascii_header_get (header, "ANT_ID2", "%d", &ant_id2) != 1)
  {     
    fprintf(stderr, "could not read ANT_ID2 from header2\n");
    return -1;    
  }                 
...
*/
  ...
  size_t unpacked_size = ndat_overlap * sizeof(float);
  float * unpacked1 = (float *) fftwf_malloc (unpacked_size);
  float * unpacked2 = (float *) fftwf_malloc (unpacked_size);
  ...
  size_t nbytes_freq = sizeof(float) * ochan;
  float * amps   = (float *) malloc (nbytes_freq);
  float * phases = (float *) malloc (nbytes_freq);
  float * power1 = (float *) malloc (nbytes_freq);
  float * power2 = (float *) malloc (nbytes_freq);

  bzero (power1, nbytes_freq);
  bzero (power2, nbytes_freq);

  // amps and phases with time resolution
  size_t nbytes_time = sizeof(float) * nblocks;
  float * amps_t = (float *) malloc(nbytes_time);
  float * phases_t = (float *) malloc(nbytes_time);

  // FWD filterbank plan TODO R2C
  fftwf_plan plan_fwd  = fftwf_plan_dft_r2c_1d (npt, unpacked1, spectrum1, FFTW_ESTIMATE);
  ...
  const double channel_bw = (double) 0.5 / tsamp;
  ...
  for (iblock=0; iblock<nblocks; iblock++)
  {
    ...
    int d1 = 0;
    int d2 = 0;
    float * delayed1 = unpacked1 + d1;
    float * delayed2 = unpacked2 + d2;
...
//#define USE_FIR
    ...
    {
      fftwf_execute_dft_r2c (plan_fwd, in1, out1);
      fftwf_execute_dft_r2c (plan_fwd, in2, out2);
      ...
      {
        // normalise 
        const complex float p1 = out1[ipt] / ochan;
        const complex float p2 = out2[ipt] / ochan;
    ...
    {
      unsigned idx = ipt;
      unsigned odx = (idx + (ochan/2)) % (ochan);
    ...
    float chanbw = (float) (channel_bw / ochan);
    float basefreq = (float) (freq - (bw / 2));
    plot_cross_vs_freq (amps, phases, ochan, "3/xs", chanbw, basefreq);
...
void plot_cross_vs_freq (float * cross, float * phases, unsigned npts, char * device, float chanbw, float base_freq)
{
  if (cpgbeg(0, device, 1, 1) != 1)
  {
    fprintf(stderr, "error opening plot device\n");
  ...
  float xmin = base_freq;
  float xmax = base_freq + (float) npts * chanbw;
  float ymin = FLT_MAX;
  float ymax = -FLT_MAX;
  ...
  for (i=0; i<npts; i++)
  {
    ...
    xvals[i] = base_freq + (float) i * chanbw;
  }
