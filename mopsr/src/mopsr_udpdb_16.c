...
time_t mopsr_udpdb_start (dada_pwc_main_t * pwcm, time_t start_utc)
{
  ...
  if (ascii_header_set (pwcm->header, "CHAN_OFFSET", "%d", 0) < 0)
  {
                                ...
                                uint64_t block_size, uint64_t block_id)
{
  ...
  uint64_t seq_no, seq_byte, byte_offset, expected_seq, iseq;
  ...
  while (!ctx->got_enough)
  {
    ...
    if (!ctx->got_enough && ctx->sock->have_packet)
    {
      ...
      if ((!ctx->capture_started) && (ctx->hdr.seq_no < 10000))
      {
        // TODO double check with Tim that first packet has sequence number of 1
        ctx->prev_seq = 0;
        ctx->start_byte = 1 * ctx->pkt_size;
        ctx->end_byte   = (ctx->start_byte + block_size) - ctx->pkt_size;
        //if (ctx->verbose)
          multilog (pwcm->log, LOG_INFO, "recv_block: START [%"PRIu64" - %"PRIu64"] seq_no=%"PRIu64"\n", ctx->start_byte, ctx->end_byte, ctx->hdr.seq_no);
      ...
      if (ctx->capture_started)
      {
        ...
        // loop for handling zeroed packets
        for (iseq = expected_seq; (iseq <= ctx->hdr.seq_no && block_count < ctx->packets_per_buffer); iseq++)
        {
          seq_byte = iseq * ctx->pkt_size;

          // if this "packet" belonged in a previous buffer, just ignore it
          if (seq_byte < ctx->start_byte)
          {
            ctx->sock->have_packet = 0;
          }
          // this "packet" belongs in the current buffer
          else if (seq_byte <= ctx->end_byte)
          {
            ...
            // if we are padding in a dropped packet
            if (iseq < expected_seq)
            {
              multilog (pwcm->log, LOG_INFO, "recv_block: dropped %"PRIu64" packets\n", expected_seq - iseq);
              memcpy (block + byte_offset, ctx->zeroed_packet, ctx->pkt_size);
              ctx->packets->dropped++;
              ctx->bytes->dropped += ctx->pkt_size;
            }
            // otherwise we are handling a legit packet
            else if (iseq == expected_seq)
            {
              byte_offset = seq_byte - ctx->start_byte;
              if (ctx->zeros)
                memcpy (block + byte_offset, ctx->zeroed_packet, ctx->pkt_size);
          ...
          // packet belongs in a future block, end loop
          else
          {
            ctx->got_enough = 1;
          }
  ...
  if (ctx->verbose)
    multilog (pwcm->log, LOG_INFO, "block_count=%"PRIu64" pkts_caught=%"PRIu64"\n", block_count, pkts_caught);
  ...
  if (block_count)
  {
    ...
    {
      multilog (pwcm->log, LOG_WARNING, "total dropped packets=%"PRIu64"\n", ctx->packets->dropped);
    }
  }
