 ...
 * mopsr_udpdb_thread
 *
 * Reads UDP packets for Molongolo Observatory PFB system:
 ...
 */
int mopsr_udpdb_init_receiver (udpdb_t * ctx)
{
  if (ctx->verbose > 1)
 ...
 */
int mopsr_udpdb_destroy_receiver (udpdb_t * ctx)
{
  if (ctx->sock)
 ...
 */
void mopsr_udpdb_reset_receiver (udpdb_t * ctx) 
{
  if (ctx->verbose)
    multilog (ctx->log, LOG_INFO, "mopsr_udpdb_reset_receiver()\n");
...
int mopsr_udpdb_prepare (udpdb_t * ctx)
{
  if (ctx->verbose)
 ...
 */
time_t mopsr_udpdb_start (udpdb_t * ctx, char * obs_header)
{
  ...
  if (mopsr_udpdb_open_buffer (ctx) < 0)
  {
    multilog (ctx->log, LOG_ERR, "start: mopsr_udpdb_open_buffer failed\n");
    return -1;
  }
 ...
 */
int mopsr_udpdb_open_buffer (udpdb_t * ctx)
{

  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "mopsr_udpdb_open_buffer()\n");
 ...
 */
int mopsr_udpdb_close_buffer (udpdb_t * ctx, uint64_t bytes_written, unsigned eod)
{

  if (ctx->verbose && stop_pending || ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "mopsr_udpdb_close_buffer(%"PRIu64", %d)\n", bytes_written, eod);
 ...
 */
int mopsr_udpdb_new_buffer (udpdb_t * ctx)
{

  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "mopsr_udpdb_new_buffer()\n");

  if (mopsr_udpdb_close_buffer (ctx, ctx->hdu_bufsz, 0) < 0)
  {
    multilog (ctx->log, LOG_INFO, "new_buffer: mopsr_udpdb_close_buffer failed\n");
    return -1;
  }

  if (mopsr_udpdb_open_buffer (ctx) < 0) 
  {
    multilog (ctx->log, LOG_INFO, "new_buffer: mopsr_udpdb_open_buffer failed\n");
    return -1;
  }
 ...
 * datablocks
 */
void * mopsr_udpdb_receive_obs (void * arg)
{
  ...
  if (ctx->verbose)
    multilog(log, LOG_INFO, "mopsr_udpdb_receive_obs()\n");
  ...
  while (!quit_threads && !stop_pending) 
  {
    ...
    if (ctx->sock->have_packet) 
    {
      ...
      if (ctx->block_count >= ctx->packets_per_buffer)
      {
        ...
        if (mopsr_udpdb_new_buffer (ctx) < 0)
        {
          multilog(ctx->log, LOG_ERR, "receive_obs: mopsr_udpdb_new_buffer failed\n");
          thread_result = -1;
          pthread_exit((void *) &thread_result);
        }
    ...
    if (stop_byte)
    {
      if (seq_byte >= stop_byte)
      {
        ...
        if (mopsr_udpdb_close_buffer (ctx, bytes_just_written, 1) < 0)
        {
          multilog(ctx->log, LOG_ERR, "receive_obs: mopsr_udpdb_close_hdu failed\n");
          thread_result = -1;
          pthread_exit((void *) &thread_result);
        }
        stop_byte = 0;
...
int main (int argc, char **argv)
{
  ...
  int inc_port = MOPSR_DEFAULT_UDPDB_PORT;

  /* multilog output port */
  int l_port = MOPSR_DEFAULT_PWC_LOGPORT;
  ...
  log = multilog_open ("mopsr_udpdb_thread", 0);
  ...
  multilog (log, LOG_INFO, "main: mopsr_udpdb_init_receiver()\n");
  if (mopsr_udpdb_init_receiver (&udpdb) < 0)
  {
  ...
  if (verbose)
    multilog(log, LOG_INFO, "main: mopsr_udpdb_prepare()\n");
  if (mopsr_udpdb_prepare (&udpdb) < 0)
  {
  ...
  {
    if (verbose)
      multilog(log, LOG_INFO, "main: mopsr_udpdb_reset_receiver()\n");
    mopsr_udpdb_reset_receiver (&udpdb);
    ...
    {
      if (verbose)
        multilog(log, LOG_INFO, "main: mopsr_udpdb_start()\n");
      time_t utc = mopsr_udpdb_start (&udpdb, obs_header);
      if (utc == -1 ) {
    ...
    //rval = pthread_create (&receiving_thread_id, &recv_attr, (void *) mopsr_udpdb_receive_obs , (void *) &udpdb);
    if (verbose)
      multilog(log, LOG_INFO, "starting mopsr_udpdb_receive_obs thread\n");
    rval = pthread_create (&receiving_thread_id, 0, (void *) mopsr_udpdb_receive_obs , (void *) &udpdb);
    if (rval != 0) {
      multilog(log, LOG_INFO, "Error creating mopsr_udpdb_receive_obs thread: %s\n", strerror(rval));
      return -1;
    }

    if (verbose) 
      multilog(log, LOG_INFO, "joining mopsr_udpdb_receive_obs thread\n");
  ...
  // clean up memory 
  if ( mopsr_udpdb_destroy_receiver (&udpdb) < 0) 
