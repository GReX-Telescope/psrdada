  ...
  while (!quit_threads)
  {
    ...
    if (ctx->sock->have_packet)
    {
      ...
      else
      {
        ...
        {
          uint64_t n_pkts_dropped = seq_no - (ctx->prev_seq_no + 1);
          ctx->packets->dropped += n_pkts_dropped;
          ctx->bytes->dropped += n_pkts_dropped * UDP_DATA;
        }
        else
        {
          multilog (ctx->log, LOG_INFO, "read: out of order packets - should be impossible on direct connection!\n");
          quit_threads = 1;
        }
      }

      ctx->prev_seq_no = seq_no;
    }
  }
