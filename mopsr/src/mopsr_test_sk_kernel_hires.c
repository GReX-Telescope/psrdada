...
int main (int argc, char **argv)
{
  ...
  multilog (log, LOG_INFO, "main memory_stride=%ld\n", memory_stride);
  for (i=0; i<MOPSR_MEMORY_BLOCKS;i++)
  {
    hires_test_skcompute (stream, d_in, d_s1 + memory_offset, d_s2, nchan, nant, nbytes);
  ...
  for (iblock=0; iblock<n_memory; iblock++)
  {
    in = (float * ) h_in;
    outs2 = (float * ) h_s2;
    for (ichan=0; ichan<nchan; ichan++)
      ...
      for (iant=0; iant<nant; iant++)
      {
        for (isamp=0; isamp<nsamp; isamp+=1024)
        {
          ...
          if (((s1 > gpu_s1) && (s1 / gpu_s1 > tol)) || ((gpu_s1 > s1) && (gpu_s1 / s1 > tol)))
            printf ("[%d][%d][%d][%d] S1 %f != %f\n", iblock, ichan, iant, isamp/1024, s1, gpu_s1);
          if (((s2 > gpu_s2) && (s2 / gpu_s2 > tol)) || ((gpu_s2 > s2) && (gpu_s2 / s2 > tol)))
            printf ("[%d][%d][%d][%d] S2 %f != %f\n", iblock, ichan, iant, isamp/1024, s2, gpu_s2);
  ...
  hires_test_compute_power_limits (stream, d_s1, d_sigmas, d_thresh, d_mask, nsums, nant, nchan, ndat_cpl, s1_count, s1_memory, d_rstates);
  ...
  unsigned nelements = nsums *  n_memory;
  outs1 = (float * ) h_s1;
  float * h_thr = (float *) h_thresh;
  float * tmp = (float *) malloc(sizeof(float) * nelements);
  ...
  for (ichan=0; ichan<nchan; ichan++)
  {
    for (iant=0; iant<nant; iant++)
    {
      outs1 = ((float * ) h_s1) + (ichan * nant * 16) + (iant * 16);

      // now read 64 x 16 samples in blocks of FST subsets
      // stored a 64 sets of 16 samples in FST subsets
      unsigned idx = 0;
      unsigned odx = 0;
      for (iblock=0; iblock<n_memory; iblock++)
      {
      ...
      qsort ((void*) tmp, nelements, sizeof(float), float_compare);

      float median = tmp[nelements/2];

      for (i=0; i<nelements; i++)
        tmp[i] = fabsf(tmp[i] - median);

      qsort ((void *) tmp, nelements, sizeof(float), float_compare);
      float stddev = tmp[nelements/2] * 1.4826;
      ...
      if (((upper > gpu_upper) && (upper / gpu_upper > tol)) || ((gpu_upper > upper) && (gpu_upper / upper > tol)))
        fprintf (stderr, "[%d][%d] [%f - %f] median=%f sigma=%f || [%f - %f] median=%f sigma=%f\n",
                ichan, iant, upper, lower, 
                median, stddev,
                gpu_upper, gpu_lower, gpu_median, gpu_sigma);
  ...
  hires_test_skdetect (stream, d_s1, d_s2, d_thresh, d_mask, d_sigmas, nsums, nant, nchan, nsamp);
  ...
  for (ichan=0; ichan<nchan; ichan++)
  {
    ...
    { 
      ...
      gpu_upper = gpu_median + 3 * gpu_sigma;
      gpu_lower = gpu_median - 3 * gpu_sigma;
      ...
      for (isum=0; isum<nsums; isum++)
      {
        ...
        if (sk < sk_l || sk > sk_h || s1 < gpu_lower || s1 > gpu_upper)
        {
          if (*h_ptr == 0)
            if (verbose)
              fprintf (stderr, "[%d][%d][%d] FP s1=%f s2=%f sk=%f [%f - %f] thresh[%f - %f] mask=%d\n", ichan, iant, isum, s1, s2, sk, sk_l, sk_h, gpu_upper, gpu_lower, *h_ptr);
        }
        else
        {
          if (*h_ptr != 0)
            if (verbose)
              fprintf (stderr, "[%d][%d][%d] FN s1=%f s2=%f sk=%f [%f - %f] thresh[%f - %f] mask=%d\n", ichan, iant, isum, s1, s2, sk, sk_l, sk_h, gpu_upper, gpu_lower, *h_ptr);
        }
        h_ptr ++;
        outs1 ++;
        outs2 ++;
...
#ifdef USE_CONSTANT_MEMORY
  mopsr_delay_copy_scales (stream, h_ant_scales, ant_scales_size);
  hires_test_skmask (stream, d_in, d_out, d_mask, d_rstates, d_sigmas, nsums, nchan, nant, nsamp, replace_noise);
#else
  error = cudaMemcpyAsync (d_ant_scales, h_ant_scales, ant_scales_size, cudaMemcpyHostToDevice, stream);
  hires_test_skmask (stream, d_in, d_out, d_mask, d_rstates, d_sigmas, d_ant_scales, nsums, nchan, nant, nsamp, replace_noise);
#endif
