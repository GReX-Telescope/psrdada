...
int main(int argc, char** argv) 
{
  ...
  while ( ipt < npts )
  {
    ...
    // calculate the effect of this slope error the first observation
    // which would have been absorbed into the calibration
    ...
    for (i=0; i<nsources; i++)
    {
      ...
      for (imod=0; imod<nmod; imod++)
      {
        // determine the additional error that arises from slope vs the default
        float fringe1 = delays1[imod]->fringe_coeff;
        float fringe2 = delays2[imod]->fringe_coeff;

        // the difference is the error of (slope - default) 
        float difference = fringe2 - fringe1;

        // now we substract this difference in phase offet to the measured phase offset 
        results[i][imod] = (phases[i][imod].phase_offset - difference) - correction[imod];
        ...
        if (phases[i][imod].scale > 0.4 && fabsf(phases[i][imod].phase_offset) < 1)
        {
          // -ve east, +ve west
      ...
      gsl_fit_wlinear (x_w, 1, w_w, 1, y_w, 1, (size_t) nvalid_w, &c0_w, &c1_w, &cov00, &cov01, &cov11, &chisq);
      gsl_fit_wlinear (x_e, 1, w_e, 1, y_e, 1, (size_t) nvalid_e, &c0_e, &c1_e, &cov00, &cov01, &cov11, &chisq);

      mopsr_phase_delays_plot (dists, results[i], 352, titles[i], devices[i], c1_w, c0_w, c1_e, c0_e);
