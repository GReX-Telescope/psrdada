...
int mopsr_udpdb_init (mopsr_udpdb_t * ctx) 
{
  ...
  ctx->end_chan   = 64;
  ctx->old_nchan  = 128;
  ctx->new_nchan  = (ctx->end_chan - ctx->start_chan) + 1;
  ctx->pkt_size   = (UDP_DATA * ctx->new_nchan) / ctx->old_nchan;
...
time_t mopsr_udpdb_start (dada_pwc_main_t * pwcm, time_t start_utc)
{
  ...
  if (ascii_header_get (pwcm->header, "NANT", "%d", &(ctx->nant)) != 1)
  {
    multilog (pwcm->log, LOG_WARNING, "start: failed to read NANT from header\n");
  }
  if (ctx->verbose)
    multilog (pwcm->log, LOG_INFO, "start: NANT=%d\n", ctx->nant);

  unsigned int old_nchan;
  if (ascii_header_get (pwcm->header, "NCHAN", "%d", &old_nchan) != 1)
  {
    multilog (pwcm->log, LOG_WARNING, "start: failed to read NCHAN from header\n");
  }
  if (ctx->verbose)
    multilog (pwcm->log, LOG_INFO, "start: old NCHAN=%d\n", old_nchan);
  ...
  float chan_bw  = old_bw / old_nchan;
  ...
  float new_start_chan_cfreq = freq_low + (ctx->start_chan * chan_bw) + (chan_bw / 2);
  float new_end_chan_cfreq   = freq_low + (ctx->end_chan * chan_bw) + (chan_bw / 2);
  if (ctx->verbose)
  {
  ...
  new_bytes_per_second = (old_bytes_per_second * ctx->new_nchan) / ctx->old_nchan;
  new_resolution = (old_resolution * ctx->new_nchan) / ctx->old_nchan;

  // set the channel offset in the header
  if (ascii_header_set (pwcm->header, "CHAN_OFFSET", "%d", ctx->start_chan) < 0)
  {
                                ...
                                uint64_t block_size, uint64_t block_id)
{
  ...
  int8_t * in, * out;
  uint64_t in_offset, in_stride;
  uint64_t out_stride;
  unsigned int iframe;

  in = (void *) ctx->sock->buf + UDP_HEADER;
  in_offset = ctx->start_chan * ctx->nant * 2;
  in_stride = ctx->old_nchan * ctx->nant * 2;
  out_stride = ctx->new_nchan * ctx->nant * 2;
    ...
    if (!ctx->got_enough && ctx->sock->have_packet)
    {
      ...
      if (ctx->capture_started)
      {
        ...
        else if (ctx->hdr.seq_no <= ctx->end_pkt)
        {
          ...
          {
            in = (void *) ctx->sock->buf + UDP_HEADER + in_offset;
            out = block + byte_offset;
