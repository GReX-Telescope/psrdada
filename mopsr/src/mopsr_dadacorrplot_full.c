...
int main (int argc, char **argv)
{
  ...
  if (delay)
  {
    if (delay > 0)
    {
      aff_t offset = delay * samp_stride;
...
*/
  ...
  //modules[1].fixed_delay += (double) ((fractional_delay * tsamp) / 1e6);
  //modules[0].fixed_delay = 0.00001021;
  //modules[1].fixed_delay = 0.00001021;

  fprintf (stderr, "fractional_turns=%lf\n", fractional_turns); 
  modules[1].fixed_delay += (double) ((fractional_turns * (1.0 / bw)) / 1e6);

  fprintf (stderr, "input fixed_delays=%lf %lf\n", modules[0].fixed_delay, modules[1].fixed_delay);
  ...
  for (iblock=0; iblock<nblocks; iblock++)
  {
    ...
    //fill_gaussian_chars (raw1, raw_size, 8, 500);
    //memcpy (raw2, raw1, raw_size);
