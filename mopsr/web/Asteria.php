...
$q = 'SELECT name, dm, period, max_snr_in5min, utc, snr, tint/60. as tint, psr_id FROM (Pulsars JOIN UTCs JOIN Observations ON Pulsars.id = Observations.psr_id AND UTCs.id = Observations.utc_id) WHERE tint > 0.5 ORDER BY utc DESC LIMIT 100';
#$q = 'SELECT name, dm, period, max_snr_in5min, utc, snr, tint/60. as tint, psr_id FROM Pulsars JOIN UTCs JOIN Observations ON Pulsars.id = Observations.psr_id AND UTCs.id = Observations.utc_id WHERE tint > 0.5 ORDER BY utc DESC LIMIT 100';
...
foreach ($results as $row)
{
  ...
  if (round($snr_5m, 2) == round($max_snr, 2)) {
    $q = 'SELECT snr, tint/60. as tint FROM Observations WHERE psr_id = '.$psr_id.' AND tint > 0.5;';
    $stmt = $pdo -> query($q);
