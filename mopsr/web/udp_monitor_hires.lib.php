...
class udp_monitor extends mopsr_webpage 
{
  ...
  var $modules_per_pfb = 8;

  var $pfb_suffixes = array("B", "R");
...
?>
    <script type='text/javascript'>  
      ...
      {
        var url = "udp_monitor_hires.lib.php?update=true";
      ...
      function udp_update_secs()
      {
        ...
        document.location = "udp_monitor_hires.lib.php?single=true&update_secs="+update_secs;
      }
...
<?
    ...
    for ($iarm=0; $iarm < count($this->arm_prefixes); $iarm++)
    {
      ...
      {
        foreach ($this->pfb_suffixes as $suffix)
        {
          $pfb_id = sprintf("%s%02d", $this->arm_prefixes[$iarm], ($irow+1))."-".$suffix;
          ...
          for ($imod = 0; $imod < $this->modules_per_pfb; $imod++)
          {
            ...
            echo "<td>";
            echo "<a href='udp_input_hires.lib.php?single=true&pfb_id=".$pfb_id."&pfb_name=".$pfb_id."'>";
