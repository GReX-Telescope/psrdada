...
###############################################################################
#
# client_mopsr_bp_smirf.pl 
#
# run multibeam smirf on the reblocked, 8-bit unsigned ints
# 
###############################################################################
...
#
{
  ...
  while (!$quit_daemon)
  {
    ...
    $proc_cmd = "java -jar ".$smirf_binary;
    ...
    else
    {
      ...
      # smirf produces output in the current working directory
      ...
      # starting beam for this smirf instance
      my $start_beam = 1 + $ct{"BEAM_FIRST_RECV_".$proc_id};

      if ($hires)
      {
        $proc_cmd = "smirf -k ".$db_key." -gpu_id ".$cfg{"BP_GPU_ID_".$proc_id}." -dm 0 2000 -dm_tol 1.25 -boxcar_max 4096 -beam ".$start_beam." -output_dir ".$proc_dir;
        $proc_cmd = "smirf -k ".$db_key." -gpu_id ".$cfg{"BP_GPU_ID_".$proc_id}." -dm 0 2000 -dm_tol 1.25 -boxcar_max 2048 -beam ".$start_beam." -coincidencer ".$cfg{"SERVER_HOST"}.":".$cfg{"COINCIDENCER_PORT"};
        $proc_cmd = "smirf -k ".$db_key." -gpu_id ".$cfg{"BP_GPU_ID_".$proc_id}." -dm 0 2000 -dm_tol 1.20 -boxcar_max 1024 -beam ".$start_beam." -coincidencer ".$cfg{"SERVER_HOST"}.":".$cfg{"COINCIDENCER_PORT"};
      }
      else
      {
        $proc_cmd = "smirf -k ".$db_key." -gpu_id ".$cfg{"BP_GPU_ID_".$proc_id}." -dm 0 2000 -dm_tol 1.05 -boxcar_max 4096 -beam ".$start_beam." -output_dir ".$proc_dir;
        $proc_cmd = "smirf -k ".$db_key." -gpu_id ".$cfg{"BP_GPU_ID_".$proc_id}." -dm 0 2000 -dm_tol 1.05 -boxcar_max 4096 -beam ".$start_beam." -coincidencer ".$cfg{"SERVER_HOST"}.":".$cfg{"COINCIDENCER_PORT"};
      }
      ...
      ($result, $response) = Dada::mySystemPiped($cmd, $src_log_file, $src_log_sock, "src", sprintf("%02d",$proc_id), $daemon_name, "bp_smirf");
      msg(1, "INFO", "END   ".$cmd);
      if ($result ne "ok")
      {
        $quit_daemon = 1;
...
sub msg($$$)
{
  ...
  if ($level <= $dl)
  {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, sprintf("%02d",$proc_id), $time, "sys", $type, "bp_smirf", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
