...
#
{
  ...
  while (!$quit_daemon)
  {
    ...
    else
    {
      ...
      logMsg(2, "INFO", "Full path to BP_PROC_FILE: ".$proc_cmd_file);
      if ( ! ( -f $proc_cmd_file ) ) 
      {
        logMsg(0, "ERROR", "BP_PROC_FILE did not exist: ".$proc_cmd_file);
        $proc_cmd = "dada_dbnull -z -s -k ".$db_key;
      }
      elsif ($cfg{"BP_STATE_".$proc_id} eq "active")
      {
        logMsg(1, "INFO", "BP_PROC_FILE=".$proc_cmd_file);
      ...
      {
        logMsg(0, "INFO", "BP_STATE_".$proc_id." == ".$cfg{"BP_STATE_".$proc_id});
        $proc_cmd = "dada_dbnull -z -s -k ".$db_key;
        logMsg(1, "INFO", "PROC_CMD=".$proc_cmd);
      }
      ...
      if ($result ne "ok")
      {
        logMsg(0, "ERROR", "could not create local dir for proc_id=".$proc_id.": ".$response);
        $proc_cmd = "dada_dbnull -z -s -k ".$db_key;
        $proc_dir = "/";
      }
      else
      ...
      ($result, $response) = Dada::mySystemPiped($cmd, $src_log_file, $src_log_sock, "src", $proc_id, $daemon_name, "bp_proc");
...
sub logMsg($$$)
{
  ...
  if ($level <= $dl)
  {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, $proc_id, $time, "sys", $type, "bp_proc", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
