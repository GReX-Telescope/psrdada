...
#
{
  ...
  my $can_dump = 1;
  my $dump_dir = $cfg{"CLIENT_DUMP_DIR"}."/".$pfb;

  logMsg(2, "INFO", "main: dump_dir=".$dump_dir);
  if (! -d $dump_dir)
  {
    logMsg(2, "INFO", "main: dump_dir did not exist, creating");
    ($result, $response) = Dada::mkdirRecursive ($dump_dir, 0755);
    if ($result ne "ok")
    {
      logMsg(0, "ERROR", "Could not create dump_dir");    
      $quit_daemon = 1;
    }
    else
    {
      logMsg(2, "INFO", "main: dump_dir created");
    }
  }
  ...
  if ($result ne "ok")
  {
    logMsg(0, "ERROR", "Could not write to dump_dir"); 
    $quit_daemon = 1;
  }
  else
  {
  ...
  while (!$quit_daemon) 
  {
    ...
    else
    {
      ...
      if ($can_dump)
      {
        $cmd = "dada_dbdisk -k ".$db_key." -D ".$dump_dir." -s -o";
      }
      else
      {
...
sub logMsg($$$) {
  ...
  if ($level <= $dl) {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, sprintf("%02d",$pwc_id), $time, "sys", $type, "dump", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
