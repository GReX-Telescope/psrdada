...
#
{
  ...
  my $aq_dir = $cfg{"CLIENT_RESULTS_DIR"}."/".$cfg{"PWC_PFB_ID_".$pwc_id};
  my $user = "dada";
  ...
  while (!$quit_daemon) 
  {
    $cmd = "find ".$aq_dir." -mindepth 2 -maxdepth 2 -type f -name 'aqdsp.finished' | awk -F/ '{print \$(NF-1)}'";
    msg(2, "INFO", "main: ".$cmd);
    ...
    if (($result eq "ok") && ($response ne ""))
    {
      ...
      for ($i=0; (!$quit_daemon && $i<=$#utcs); $i++)
      {
        $utc_start = $utcs[$i];

        $cmd = "find ".$aq_dir."/".$utc_start." -mindepth 1 -maxdepth 1 -type f -name '*.mask' | wc -l";
        msg(2, "INFO", "main: ".$cmd);
        ($result, $response) = Dada::mySystem($cmd);
        msg(3, "INFO", "main: ".$result." ".$response);
        ...
        {
          $cmd = "mkdir -p ".$cfg{"SERVER_RESULTS_DIR"}."/".$utc_start."/masks";
          ...
          { 
            $cmd = "rsync -a --stats --bwlimit=2048 --no-g --chmod=go-ws --password-file=/home/mpsr/.ssh/rsync_passwd ".
                   $aq_dir."/".$utc_start."/*.mask upload\@172.17.228.204::results/".$utc_start."/masks/";
            ...
            else
            {
              ...
              $cmd = "mv ".$aq_dir."/".$utc_start."/aqdsp.finished ".$aq_dir."/".$utc_start."/aqdsp.transferred";
              msg(2, "INFO", "main: ".$cmd);
              ($result, $response) = Dada::mySystem($cmd);
              msg(3, "INFO", "main: ".$result." ".$response);
              if ($result ne "ok")
              ...
              {
                $cmd = "rm -f ".$aq_dir."/".$utc_start."/*.mask";
                msg(2, "INFO", "main: ".$cmd);
                ($result, $response) = Dada::mySystem($cmd);
                msg(3, "INFO", "main: ".$result." ".$response);
              }
        ...
        {
          $cmd = "mv ".$aq_dir."/".$utc_start."/aqdsp.finished ".$aq_dir."/".$utc_start."/aqdsp.nomasks";
          msg(2, "INFO", "main: ".$cmd);
          ($result, $response) = Dada::mySystem($cmd);
          msg(3, "INFO", "main: ".$result." ".$response);
          if ($result ne "ok")
...
sub msg($$$) {
  ...
  if ($level <= $dl) {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, sprintf("%02d",$pwc_id), $time, "sys", $type, "aqmask", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
