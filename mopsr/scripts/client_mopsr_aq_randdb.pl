...
###############################################################################
#
# client_mopsr_aq_randdb.pl 
#
# generate random number sequence for testing correlation
# 
###############################################################################
...
#
{
  ...
  $cmd = "mopsr_randdb -k ".$db_key." /data/mopsr/scratch/".$pfb_id."/obs.header -s 1";

  msg(1, "INFO", "START ".$cmd);
  ($result, $response) = Dada::mySystemPiped ($cmd, $src_log_file, $src_log_sock, "src", sprintf("%02d",$pwc_id), $daemon_name, "aq_randdb");
  if ($result ne "ok")
  {
...
sub msg($$$)
{
  ...
  if ($level <= $dl)
  {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, sprintf("%02d",$pwc_id), $time, "sys", $type, "aq_randdb", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
