...
###############################################################################
#
# client_mopsr_bp_recv.pl 
#
# Receiver daemon for the Beam-Former to Beam-Processor cornerturn
# 
###############################################################################
...
sub usage() 
{
  print "Usage: ".basename($0)." RECV_ID\n";
}
...
# ensure that our recv_id is valid 
if (($recv_id >= 0) &&  ($recv_id < $ct{"NRECV"}))
{
  # and matches configured hostname
  if ($ct{"RECV_".$recv_id} ne Dada::getHostMachineName())
  {
    print STDERR "RECV_".$recv_id."[".$ct{"RECV_".$recv_id}."] did not match configured hostname [".Dada::getHostMachineName()."]\n";
    usage();
    exit(1);
  }
}
...
{
  print STDERR "recv_id was not a valid integer between 0 and ".($ct{"NRECV"}-1)."\n";
  usage();
  exit(1);
}
...
#
Dada::preventDuplicateDaemon(basename($0)." ".$recv_id);
...
#
{
  ...
  $sys_log_file = $cfg{"CLIENT_LOG_DIR"}."/".$daemon_name."_".$recv_id.".log";
  $src_log_file = $cfg{"CLIENT_LOG_DIR"}."/".$daemon_name."_".$recv_id.".src.log";
  my $pid_file =  $cfg{"CLIENT_CONTROL_DIR"}."/".$daemon_name."_".$recv_id.".pid";

  # this is data stream we will be reading from
  $db_key = Dada::getDBKey($cfg{"DATA_BLOCK_PREFIX"}, $recv_id, $cfg{"NUM_BP"}, $cfg{"BP_DATA_BLOCK"});
  ...
  while (!$quit_daemon)
  {
    ...
    ($result, $response) = Dada::mySystemPiped($cmd, $src_log_file, $src_log_sock, "src", $recv_id, $daemon_name, "bp_recv");
...
sub logMsg($$$)
{
  ...
  if ($level <= $dl)
  {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, $recv_id, $time, "sys", $type, "bp_recv", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
...
sub controlThread($)
{
  ...
  my $pwc_quit_file  = $cfg{"CLIENT_CONTROL_DIR"}."/".$daemon_name."_".$recv_id.".quit";
