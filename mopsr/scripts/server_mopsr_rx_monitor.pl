...
###############################################################################
#
# server_mopsr_rx_monitor.pl
#
# This manages the interface between the RX subsystems and the web monitor
#
#   * plots a list of low res histograms for every RX board
#   * manages temporary .bin files dumpe
#   * maintains list of low res histograms from every RX board
#   * 
...
#
{
  ...
  $sock = new IO::Socket::INET (
    ...
    LocalPort => $cfg{"SERVER_RX_MONITOR_PORT"},
    Proto => 'tcp',
    Listen => 1,
    Reuse => 1,
  );
  ...
  my $raw_monitor_thread = 0;

  # Launch monitoring threads
  $raw_monitor_thread = threads->new(\&rawMonitorThread, "/data/mopsr/server/monitor");

  $rx_monitor_thread = threads->new(\&rxMonitorThread, "/data/mopsr/server/rx_monitor");

  # Sleep for a few seconds to allow the threads to start and collect
  # their first iteration of data
  sleep(3);

  my ($command, $xml, $xml_out);
    ...
    foreach $rh (@$rh_set) 
    {
      ...
      else
      {
        ...
        else
        {
          ...
          {
            $result = "fail";
            $resposne = "failed to parse xml";
          }
          else
          {
            my $has_command = eval { exists $xml->{'command'} };
              
            if ($command eq "rx_monitor_info")
            {
          ...
          elsif ($string eq "raw_monitor_info") { $r = $raw_monitor_string; }
...
#
sub rawMonitorThread ($)
{
  my ($monitor_dir) = @_;

  Dada::logMsg(1, DL, "rawMonitorThread: starting");
    ...
    else
    {
      ...
      foreach $ant ( @ants )
      {
        ...
        foreach $key ( keys  %{$to_use{$ant}})
        {
          ...
          $xml .= "<img type='".$type."' width='".$xres."' height='".$yres."'>monitor/".$to_use{$ant}{$key}."</img>";
        }
        $xml .= "</ant>\n";
      }

      $raw_monitor_string = $xml;
      Dada::logMsg(2, DL, "rawMonitorThread: collected images");
    }

    if ($img_clean_counter <= 0)
    {
      Dada::logMsg(2, DL, "rawMonitorThread: removeOldFiles");
      removeOldFiles($monitor_dir, 0, "png");
      $img_clean_counter = $img_clean_time;
    }
  }

  Dada::logMsg(1, DL, "rawMonitorThread: exiting");
}
  ...
  while (!$quit_daemon)
  {
    ...
    else
    {
      ...
      foreach $rx ( keys %bins_to_use )
      {
        ...
        if ($result ne "ok")
        {
          Dada::logMsgWarn($warn, "rxMonitor: could not plot ".$bin);
        }
      }
  ...
  while (!$quit_daemon)
  {
    ...
    else
    {
      ...
      if ($result eq "ok")
      {
        ...
        # get the listing of image files
        $cmd = "find ".$results_dir."/".$utc_start." -ignore_readdir_race -mindepth 1 -mindepth 1 -name '*.*.??.*x*.png' | sort -n | awk -F/ '{print \$(NF)-1\"\/\"\$(NF)}'";
        $img_string = `$cmd`;
        @images = split(/\n/, $img_string);
...
sub removeOldFiles($$$)
{
  ...
  for ($i=0; $i<=$#images; $i++)
  {
    ...
    {
      # only delete if > 20 seconds old
      if (($time_unix + 20) < $now)
      {
        #Dada::logMsg(1, DL, "removeOldFiles: deleteing ".$dir."/".$images[$i].", duplicate, age=".($now-$time_unix));
        unlink $dir."/".$images[$i];
      }
      else
      {
        #Dada::logMsg(1, DL, "removeOldFiles: keeping ".$dir."/".$images[$i].", duplicate, age=".($now-$time_unix));
      }
    }
    else
    {
      #Dada::logMsg(1, DL, "removeOldFiles: keeping ".$dir."/".$images[$i].", newest, age=".($now-$time_unix));
    }
  }
}
