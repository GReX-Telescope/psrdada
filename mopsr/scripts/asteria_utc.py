...
INSERT INTO TB_Obs (psr_id, utc_id, snr, tint, bw, freq, nbin, MJD_start)
  VALUES (%s, %s, %s, %s, %s, %s, %s, %s) ON DUPLICATE KEY UPDATE snr = %s, tint = %s, bw = %s, freq = %s, nbin = %s, MJD_start = %s
'''

update_all_meta_Obs = '''
UPDATE TB_Obs SET psr_id = %s, snr = %s, tint = %s, bw = %s, freq = %s, nbin = %s, MJD_start = %s WHERE id = %s
'''
...
# in most cases the UTC will be new hence probably insert or ignore is more efficient than first checking.
...
def insert_psr_into_DB(psr, cur, conn):
  cmd = "/home/dada/linux_64/bin/psrcat -all -nohead -nonumber -c 'DM P0' " + psr
...
def extract_snr_tint_bw_freq_nbin_int0mjd(dir, archive_fn, verbose):
  ...
  if archive:
    ...
    #cmd = ['/home/dada/linux_64/bin/psrstat', '-c', 'snr=pdmp,snr,length', '-qQ', '-j', 'DFTp',
    cmd = ['/home/dada/linux_64/bin/psrstat', '-c', 'snr=pdmp,snr,length,bw,freq,nbin,int[0]:mjd', '-qQ', '-j', 'DFTp',
        archive_fn]
    if verbose:
    ...
    return float(out[0]), float(out[1]), float(out[2]), float(out[3]), int(out[4]),out[5]
  else:
    if verbose:
...
def insert_obs_into_DB(psr, UTC, snr, tint, cur):
  ...
  conn.commit() # REMOVE ME, only for testing, for performance better commit once

def insert_full_obs_into_DB(id_last, psr, UTC, snr, tint, bw, freq, nbin, mjd_start, cur):
  cur.execute('SELECT id FROM Pulsars WHERE name = %s', [psr, ])
  psr_id = cur.fetchone()[0]

#INSERT INTO TB_Obs (psr_id, utc_id, snr, tint, bw, freq, nbin, MJD_start) ON DUPLICATE UPDATE
  #cur.execute( set_all_meta_Obs, [psr_id, utc_id, snr, tint, bw, freq, nbin, mjd_start, snr, tint, bw, freq, nbin, mjd_start])
#UPDATE TB_Obs SET psr_id = %s, snr = %s, tint = %s, freq = %s, nbin = %s, MJD_start = %s WHERE id = %s
  cur.execute( update_all_meta_Obs, [psr_id, snr, tint, bw, freq, nbin, mjd_start, id_last])
  conn.commit
...
def replace_max_snr(psr, snr, utc_id, cur, verbose):
  cur.execute('UPDATE Pulsars SET max_snr_in5min = %s, max_snr_obs_id = %s WHERE name = %s', [snr, utc_id, psr, ])
...
def parse_infos(top_dir, UTC, utc_id, cur, config_dir, verbose):
  ...
  if len(obs_info_fns) == 1:
    ...
    accepted_keys = np.loadtxt(config_dir + "/obs.info_params.only_common", dtype='str')
    ...
    if "CORR_ENABLED" in infos:
      if infos["CORR_ENABLED"] == "true":
        if "CORR_SOURCE" in infos:
          ...
          parse_header(src, src, top_dir, utc_id, cur, config_dir + "/obs.corr_header_params.only_common", "Corr_Obs", "Corr_Headers", verbose)
        else:
    ...
    if "MB_ENABLED" in infos:
      if infos["MB_ENABLED"] == "true":
        if "SOURCE" in infos:
          src = infos["SOURCE"]
        parse_header(src, "FB", top_dir, utc_id, cur, config_dir + "/obs.mb_header_params.only_common", "MB_Obs", "MB_Headers", verbose)
    return 0
...
def parse_state(top_dir, utc, utc_id, cur, verbose):
  state = "unknown"
  for possible_state in ["processing", "finished", "transferred", "completed", "failed"]:
...
def parse_psr_TB(psr, top_dir, utc_dir, cur, config_dir, verbose):
  ...
  for psrfile in glob.glob(_dir + "/" + psr + "_t.tot"):
    TB_Obs_id = parse_header(psr, psr, top_dir, utc_id, cur, config_dir + "/obs.tb_header_params.only_common", "TB_Obs", "TB_Headers", verbose)
    snr, tint, bw, freq, nbin, int0mjd = extract_snr_tint_bw_freq_nbin_int0mjd(_dir, psrfile, verbose)
    insert_full_obs_into_DB(TB_Obs_id, psr, utc_id, snr, tint, bw, freq, nbin, int0mjd, cur)
...
def construct_header_query(src, utc_id, fn, allowed_keys_fn, id_table, table, verbose):
  ...
  for key in cfg.keys():
    if key in accepted_keys:
      query += "`" + key + "`,"
      #handle special case of a problem with UT1_OFFSET having two entries on 2016-09-12
      if isinstance(cfg[key], list):
        if key == "UT1_OFFSET":
          cfg[key] = cfg[key][0]
        else:
          with open("asteria_error.log", "a") as fh:
            fh.write("cfg[key] is a list" + src + " " + str(utc_id) + " " + key)
          print "cfg[key] is a list " + src + " " + str(utc_id) + " " + key
          continue
      ...
      with open("headers_missing.log", "a") as fh:
        fh.write(src + " " + UTC + " " + key + " not present in DB")
...
def parse_header(src, src_dir, top_dir, utc_id, cur, allowed_keys_fn, id_table, table, verbose):
  obs_table_prefill_query = "INSERT INTO " + id_table + " (utc_id) VALUES (" + str(utc_id) +")"
  ...
  if len(obs_head_fns) == 1:
    query = construct_header_query(src, utc_id, obs_head_fns[0], allowed_keys_fn, id_table, table, verbose)
    try:
      cur.execute(query)
    except (sql.OperationalError, sql.ProgrammingError) as err:
...
if __name__ == "__main__":
  ...
  parser.add_argument("--data-top-dir", dest="dataTopPath",
      help="Path to observations", default="/data/mopsr/results/")
...
#      help="Path to mysql config file", default="")
  ...
  # The keys and values are a bit funny because we use the php mysql config file
  conn = sql.connect(host=mysql_cfg['define("MYSQL_HOST",'].replace('");', '').lstrip('"'), db=mysql_cfg['define("MYSQL_DB",'].replace('");', '').lstrip('"').strip('"'), passwd=mysql_cfg['define("MYSQL_PWD",'].replace('");', '').lstrip('"'), user=mysql_cfg['define("MYSQL_USER",'].replace('");', '').lstrip('"'))
  cur = conn.cursor()

  # Find a list of dirs to process
  dirs = glob.glob(args.dataTopPath + "/" + args.UTC)
  if len(dirs) !=1:
    print "Found ", len(dirs), "directories  matchign the obs, expected 1"
    sys.exit(-1)
  ...
  infos_id = insert_infos_into_DB(_dir, args.UTC, utc_id, cur, args.configDir, args.Verbose)
