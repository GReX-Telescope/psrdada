...
#
{
  ...
  msg(2, "INFO", "main: log_file=".$sys_log_file);
  msg(2, "INFO", "main: pid_file=".$pid_file);
  ...
  my $event_port = (int($cfg{"CLIENT_AQ_EVENT_BASEPORT"}) + int($pwc_id));
...
sub controlThread($) 
{
  ...
  msg(2, $dl ,"controlThread: killProcess(".$cmd.", mpsr)");
  ($result, $response) = Dada::killProcess($cmd, "mpsr");
  msg(2, $dl ,"controlThread: killProcess() ".$result." ".$response);
...
sub msg($$$) {
  ...
  if ($level <= $dl) {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, sprintf("%02d",$pwc_id), $time, "sys", $type, "dbevent", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
