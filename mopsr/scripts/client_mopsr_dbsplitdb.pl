...
#
{
  ...
  # configure the datablocks to be split into
  my @proc_db_ids = split(/ /,$cfg{"PROCESSING_DATA_BLOCK"});
  my $proc_db_keys = "";
  my $proc_db_id;
  foreach $proc_db_id ( @proc_db_ids )
  {
    $proc_db_keys .= Dada::getDBKey($cfg{"DATA_BLOCK_PREFIX"}, $pwc_id, $cfg{"NUM_PWC"}, $proc_db_id)." ";
  }
  ...
  while (!$quit_daemon) 
  {
    ...
    if (!$quit_daemon)
    {
      ...
      else
      {
        ...
        ($result, $response) = Dada::mySystemPiped($proc_cmd, $src_log_file, $src_log_sock, "src", sprintf("%02d",$pwc_id), $daemon_name, "split");
...
sub controlThread($) 
{
  ...
  my $user = "mpsr";
  my $process = "^dada_header -k ".$db_key;
  ...
  push @processes_to_kill, "^mopsr_dbsplitdb ".$db_key;
