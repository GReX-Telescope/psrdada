...
#
{
  ...
  $db_key = Dada::getDBKey($cfg{"DATA_BLOCK_PREFIX"}, $send_id, $cfg{"NUM_BF"}, $cfg{"TRANSMIT_DATA_BLOCK"});
  ...
  while (!$quit_daemon)
  {
    ...
    else
    {
      ...
      ($result, $response) = Dada::mySystemPiped($cmd, $src_log_file, $src_log_sock, "src", $send_id, $daemon_name, "bp_send");
...
sub logMsg($$$)
{
  ...
  if ($level <= $dl)
  {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, $send_id, $time, "sys", $type, "bp_send", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
