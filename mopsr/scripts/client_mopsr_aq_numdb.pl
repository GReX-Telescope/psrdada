...
###############################################################################
#
# client_mopsr_aq_numdb.pl 
#
# generate stream of numbers for testing cornerturn
# 
###############################################################################
...
#
{
  ...
  my $ant_first_send = $cfg{"ANT_FIRST_SEND_".$pwc_id};

  $cmd = "/home/dada/hires/linux_64/bin/mopsr_numdb -k ".$db_key." /data/mopsr/scratch/".$pfb_id."/obs.header -n 320 -c 0 -e t -i ".$ant_first_send." -s 10";
  $cmd = "mopsr_randdb -k ".$db_key." /data/mopsr/scratch/".$pfb_id."/obs.header";

  msg(1, "INFO", "START ".$cmd);
  ($result, $response) = Dada::mySystemPiped ($cmd, $src_log_file, $src_log_sock, "src", sprintf("%02d",$pwc_id), $daemon_name, "aq_numd");
  if ($result ne "ok")
  {
...
sub msg($$$)
{
  ...
  if ($level <= $dl)
  {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, sprintf("%02d",$pwc_id), $time, "sys", $type, "aq_numdb", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
...
sub controlThread($)
{
  ...
  $cmd = "^/home/dada/hires/linux_64/bin/mopsr_numdb -k ".$db_key;
  $cmd = "mopsr_randdb -k ".$db_key;
