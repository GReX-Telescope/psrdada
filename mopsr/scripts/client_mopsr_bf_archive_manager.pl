...
###############################################################################
#
# client_mopsr_bf_archive_manager.pl 
#
...
#
{
  ...
  my $results_dir = $cfg{"CLIENT_RESULTS_DIR"}."/".$chan_tag;

  my ($cmd, $result, $response, $obs, $file, $found_something);
  my ($line, @lines, $utc_start, $i, @paths, $n, $last_obs);
  ...
  while (!$quit_daemon)
  {
    ...
    $cmd = "find ".$results_dir." -name '????-??-??-??:??:??.ar' | sort -n";
    msg(2, "INFO", "main: ".$cmd);
    ($result, $response) = Dada::mySystem($cmd);
    ...
    else
    {
      ...
      for ($i=0; (($i<=$#lines) && (!$quit_daemon)); $i++)
      {
        ...
        # since archives will be of the form . / UTC_START / FILE, grab last 2
        @paths = split(/\//, $line);
        $n = $#paths;
        ...
        msg(2, "INFO", "main: processArchive(".$chan_tag.", ".$utc_start.", ".$file.")");
        ($result, $response) = processArchive($chan_tag, $utc_start, $file);
...
sub processArchive($$$)
{
  my ($chan_tag, $utc_start, $file) = @_;

  msg(2, "INFO", "processArchive(".$chan_tag.", ".$utc_start.", ".$file.")");

  my $server = $cfg{"SERVER_HOST"};
  my $cmd = "";
  my $result = "";
  my $response = "";
  my $local_dir = $cfg{"CLIENT_RESULTS_DIR"}."/".$chan_tag."/".$utc_start;
  my $remote_dir = $cfg{"SERVER_RESULTS_DIR"}."/".$utc_start."/".$chan_tag;
  my $local_file = "";
  my $remote_file = "";
  my $i = 0;
  ...
  $local_file = $cfg{"CLIENT_RESULTS_DIR"}."/".$chan_tag."/".$utc_start."/".$file;
  $remote_file = $cfg{"SERVER_RESULTS_DIR"}."/".$utc_start."/".$chan_tag."/".$file;
  msg(2, "INFO", "processArchive: ".$local_file." -> ".$remote_file);
...
sub msg($$$)
{
  ...
  if ($level <= $dl)
  {
    ...
    if ($log_sock) {
      Dada::nexusLogMessage($log_sock, $chan_id, $time, "sys", $type, "archive mngr", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
