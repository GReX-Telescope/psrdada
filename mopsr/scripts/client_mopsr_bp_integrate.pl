...
#
{
  ...
  while (!$quit_daemon)
  {
    ...
    else
    {
      ...
      ($result, $response) = Dada::mySystemPiped($cmd, $src_log_file, $src_log_sock, "src", $proc_id, $daemon_name, "bp_int");
...
sub logMsg($$$)
{
  ...
  if ($level <= $dl)
  {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, $proc_id, $time, "sys", $type, "bp_int", $msg);
    }
    print "[".$time."] ".$msg."\n";
  }
}
