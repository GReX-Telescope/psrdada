			    ...
			    void* data, uint64_t data_size)
{
  ...
  if (dbudp->expect_header) {
    ...
    while (((data_pos + send_size) <= data_size) && (send_size > 0)) {
      ...
      if ((data_pos + UDPDATASIZE) > data_size) {
...
int main (int argc, char **argv)
{
  ...
  client->optimal_bytes = 1024*8*UDPDATASIZE;
  
  dbudp.expect_header = 1;
  dbudp.header_sent = 0;
  dbudp.port = port;
  dbudp.verbose = verbose;
  dbudp.udpbuffer = malloc(sizeof(char)*(UDPHEADERSIZE+UDPDATASIZE));
  dbudp.sequence_number = 1;
