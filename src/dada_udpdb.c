...
time_t udpdb_start_function (dada_pwc_main_t* pwcm, time_t start_utc)
{
  ...
  int verbose = pwcm->verbose;

  if (verbose) fprintf(stderr,"udpdb_start_function\n");
  ...
  if (verbose) fprintf (stderr, "udpdb_start_function returns\n");
...
void* udpdb_buffer_function (dada_pwc_main_t* pwcm, uint64_t* size)
{
  ...
  /* if (verbose) fprintf(stderr,"udpdb_buffer_function. request for %"PRIu64" bytes\n",*size); */

  /* keep receiving packets until the data buffer will not take another
     full packet */
  for (i=0; (!(udpdb->quit) && ((buffer_counter + UDPBUFFSIZE) < udpdb->datasize)) ; i++) {

    /* if we have missed udp packet(s), we need to send zero'd packets */
    if (udpdb->dropped_packets_to_fill > 0) {

      obs_data_received = UDPDATASIZE;
      ...
      /* else get a fresh packet */
        ...
        /* we need to check if data is actaully at the socket... */
        timeout.tv_sec=5;
        timeout.tv_usec=0;
        timeoutp = &timeout;
        ...
        if (rval == 0) {
          if (verbose)
            multilog (log, LOG_WARNING, "Warning. UDP packet timeout: no packet"
      ...
      if (udpdb->received != (UDPHEADERSIZE + UDPDATASIZE)) {
        ...
        if (udpdb->check_udp_headers) {
          ...
          if (!udpdb->quit) {
            multilog (log, LOG_ERR, "Error. Received %"PRIu64" bytes from udp "
                      ...
                      udpdb->received, (UDPHEADERSIZE + UDPDATASIZE));

            char zerodchar = 'c';
            memset(&zerodchar,0,sizeof(zerodchar));
            memset((udpdb->buffer+udpdb->received),zerodchar,
                   (UDPHEADERSIZE + UDPDATASIZE)-udpdb->received);
  ...
  if (udpdb->prev_time != udpdb->current_time) {
    ...
    multilog(udpdb->statslog, LOG_INFO, "%5.2fM, %"PRIu64", %5.2f GB, "
                    "%5.2f MB/s, %"PRIu64", %"PRIu64"\n",
...
int main (int argc, char **argv)
{
  ...
  log = multilog_open ("dada_udpdb", daemon);
  ...
  multilog_serve (log, DADA_DEFAULT_UDPDB_LOG);
  ...
  udpdb.current_time = udpdb.prev_time;
  udpdb.quit = 0;
  udpdb.statslog = multilog_open ("dada_udpdb_stats", 0);
  ...
  if (mode == 1) {
    ...
    if (header_file)  {
      ...
      if (ascii_header_get (header_buf, "NBAND", "%d", &(udpdb->expected_nbands)) != 1) {
        multilog (pwcm->log, LOG_WARNING, "Warning. Could not read NBAND from header, "
                  "Using 1 as default\n");
      ...
      if (ascii_header_get (header_buf, "NBIT", "%d", 
                            &(udpdb->expected_header_bits)) != 1) {
        multilog (pwcm->log, LOG_WARNING, "Warning. Could not read NBIT from header. Using "
                  "8 as default\n");
      ...
      if (ascii_header_get (header_buf, "NCHANNEL", "%d",
                            &(udpdb->expected_nchannels)) != 1) {
        multilog (pwcm->log, LOG_WARNING, "Warning. Could not read NCHANNEL from header. Using "
                  "2 as default\n");
    ...
    while (1) {
      //fprintf(stderr, "primary for loop i = %d\n",i);
      uint64_t bsize = udpdb.datasize;

      src = (char *) udpdb_buffer_function(pwcm, &bsize);

      //fprintf(stderr, "udpdb_buffer_function returned %"PRIu64" bytes\n",bsize);
      ...
      //fprintf(stderr,"ipcio_write returned\n");
...
int create_udp_socket(dada_pwc_main_t* pwcm) {
  ...
  const int pref_buffer_size = 8*1024*1024;     // socket buffer of 8MB
  ...
  len = sizeof(value);
  value = 0;
  retval = getsockopt(udpdb->fd, SOL_SOCKET, SO_RCVBUF, &value, (socklen_t *) &len);
  if (retval != 0) {
