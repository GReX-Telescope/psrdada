...
void usage ()
{
  fprintf (stdout,
	   ...
	   "WHERE:\n"
	   " -n  number of buffers in ring      [default: %llu]\n"
	   " -b  size of each buffer (in bytes) [default: %llu]\n"
...
int main (int argc, char** argv)
{
  ...
  while ((arg = getopt(argc, argv, "hdn:b:l")) != -1) {
    ...
    case 'n':
      sscanf (optarg, "%llu", &dada.nbufs);
      break;

    case 'b':
      sscanf (optarg, "%llu", &dada.bufsz);
      break;
  ...
  fprintf (stderr, "Created DADA header block with %llu bytes\n", dada.hdrsz);
