...
@r_scripts = ("caspsr_baseband_recorder");

{
  ...
  my $r_user = "caspsr";
  my $r_host = "raid0";
  my $r_path = "/lfs/data0/caspsr/control";
...
sub checkRemoteScripts($$$) 
{
  ...
  for ($i=0; $i<=$#r_scripts; $i++)
  {
    ...
    $cmd = "pgrep -u caspsr -f '^perl.*raid_".$d.".pl'";
...
sub stopRemoteScripts($$$)
{
  ...
  while (!$scripts_stopped)
  {
    ...
    for ($i=0; $i<=$#r_scripts; $i++)
    {
      ...
      $cmd = "pgrep -u caspsr -f '^perl.*raid_".$script.".pl'";
...
sub good($) {
  ...
  for ($i=0; $i<=$#backends; $i++)
  {
    ...
    $cmd = "ls -1 ".$f;
    Dada::logMsg(2, $dl, "good: remoteSshCommand($u, $h, $cmd)");
    ($result, $rval, $response) = Dada::remoteSshCommand($u, $h, $cmd);
    Dada::logMsg(2, $dl, "good: remoteSshCommand() $result $rval $response)");
    if ($result ne "ok")
    {
