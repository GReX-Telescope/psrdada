...
{
  fprintf (stdout,
	   "caspsr_dbnum_demux [options] idist chunk_size\n"
...
#define CASPSR_DBNUM_INIT { 0, 0, 0, 0, 0, 0, 0, 0 }
                  ...
                  void* data, uint64_t data_size, uint64_t block_id)
{
  ...
  for (i=0; i<seq_count; i++)
  {
    ...
    if ((i+1) % (dbnum->chunk_size / seq_size) == 0)
      dbnum->index += (dbnum->chunk_size / seq_size);
  ...
  multilog (client->log, LOG_INFO, "io_block: sleep(1) for loading of DB\n");
  sleep(1);
...
int caspsr_dbnum_open (dada_client_t* client)
{
  ...
  // move the index forward by the expected amount
  dbnum->index += (dbnum->idist * (dbnum->chunk_size / sizeof(uint64_t)));
...
int main (int argc, char **argv)
{
  ...
  if ((argc - optind) != 2) {
    fprintf (stderr, "2 command line arguments expected\n");
    usage();
    exit(EXIT_FAILURE);
  } 

  dbnum.idist = atoi(argv[optind]);
  dbnum.chunk_size = atoi(argv[optind+1]);

  log = multilog_open ("caspsr_dbnum_demux", 0);
