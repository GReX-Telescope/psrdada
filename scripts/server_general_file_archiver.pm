package Dada::server_general_file_archiver;
...
sub main() 
{
  ...
  while (!$quit_daemon) 
  {
    Dada::logMsg(2, $dl, "main: getFileToArchive(".$path.")");
    ($result, $response) = getFileToArchive ($path);
    if ($result ne "ok")
    {
      Dada::logMsg(0, $dl, "main: getFileToArchive failed: ".$response);
      $quit_daemon = 1;
      next;
    }

    my $file_to_archive = $response;

    if ($file_to_archive ne "")
    {
      # get the file size in bytes
      $cmd = "ls -s --block-size=1 ".$path."/".$file_to_archive." | awk '{print \$1}'";
      ...
      while (!$quit_daemon && $try_to_archive)
      {
        ...
        if ($try_to_archive)
        {
          my ($utc, $file) = split(/\//, $file_to_archive);
          Dada::logMsg(2, $dl, "main: tarFile(".$path.", ".$utc.", ".$file.", ".$bytes.")");
          ($result, $response) = tarFile($path, $utc, $file, $bytes);
          Dada::logMsg(2, $dl, "main: tarFile() ".$result." ".$response);
          if ($result eq "ok") 
          {
            my $dest = "on_tape";
            Dada::logMsg(2, $dl, "main: moveCompletedFile(".$path.", ".$utc.", ".$file.", ".$dest.")");
            ($result, $response) = moveCompletedFile($path, $utc, $file, $dest);
            if ($result ne "ok")
            {
              Dada::logMsg(2, $dl, "main: moveCompletedFile failed: ".$response); 
              $quit_daemon = 1;
            }
            else
            {
          ...
          else
          {
            ...
            else
            {
              $try_to_archive = 0;
              $give_up = 1;
              Dada::logMsg(0, $dl, "main: tarFile() failed: ".$response);
            }
          }
        }
      } 
...
#
sub getFileToArchive ($)
{
  Dada::logMsg(3, $dl, "getFileToArchive()");
  my ($dir) = @_;

  my ($path, $cmd, $result, $response);
  
  # format will be DIR / archive / UTC_START / FILE
  $path = $dir."/archive/";

  $cmd = "cd ".$path."; find -L . -ignore_readdir_race -mindepth 2 -maxdepth 2 -type f | sort | head -n 1";

  ($result, $response) = Dada::mySystem($cmd);

  if ($result ne "ok") 
  {
		Dada::logMsg(0, $dl, "getFileToArchive: find failed: ".$response);
  ...
  {
    my ($junk, $utc, $file) = split(/\//, $response);
    Dada::logMsg(2, $dl, "getFileToArchive: returning ".$utc."/".$file);
    return ("ok", $utc."/".$file);
  }
}
...
sub tarFile($$$$)
{
  my ($dir, $utc, $file, $est_size_bytes) = @_;

  Dada::logMsg(2, $dl, "tarFile: (".$dir.", ".$utc.", ".$file.", ".$est_size_bytes.")");
  Dada::logMsg(1, $dl, "Archiving  ".$utc."/".$file." from ".$dir);
  ...
  # Dada::logMsg(2, $dl, "tarFile: checkIfArchived(".$user.", ".$host.", ".$dir.", ".$utc.", ".$file.")");
  # ($result, $response) = checkIfArchived($user, $host, $dir, $utc, $file);
  # Dada::logMsg(2, $dl, "tarFile: checkIfArchived() ".$result." ".$response);

  # if ($result ne "ok") {
  #   Dada::logMsg(0, $dl, "tarFile: checkIfArchived failed: ".$response);
  ...
  # {
  #   Dada::logMsg(1, $dl, "Skipping archival of ".$utc."/".$file.", already archived");
  ...
  if ($result ne "ok")
  {
    Dada::logMsg(0, $dl, "tarFile: getTapeInfo() failed: ".$response);
    return ("fail", "could not determine tape information from database");
  }

  my ($id, $size, $used, $free, $nfiles, $full) = split(/:/,$response);

  Dada::logMsg(2, $dl, "tarFile: ".$free." GB left on tape");
  Dada::logMsg(2, $dl, "tarFile: size of this beam is estimated at ".$est_size_gbytes." GB");

  # if we estimate that there is not enough spce on the tape
  if ($free < $est_size_gbytes)
  {
    Dada::logMsg(0, $dl, "tarFile: tape ".$tape." full. (".$free." < ".$est_size_gbytes.")");

    # Mark the current tape as full and load a new tape;
    Dada::logMsg(2, $dl, "tarFile: updateTapesDB(".$id.", ".$size.", ".$used.", ".$free.", ".$nfiles);
    ($result, $response) = updateTapesDB($id, $size, $used, $free, $nfiles, 1);
    Dada::logMsg(2, $dl, "tarFile: updateTapesDB() ".$result." ".$response);

    if ($result ne "ok") {
      Dada::logMsg(0, $dl, "tarFile: updateTapesDB() failed: ".$response);
  ...
  while (($filenum ne $nfiles) || ($blocknum ne 0))
  {
    ...
    Dada::logMsg(0, $dl, "tarFile: WARNING: Tape out of position! ".
  ...
  while ($tries > 0) 
  {
    ...
    # different command to HRA (robot==1) tapes
		if ($robot eq 0) {
			$cmd = "cd ".$dir."; tar -h -b 128 -c ".$utc."/".$file." | dd of=".$dev." bs=64K";
		} else {
			$cmd = "cd ".$dir."; tar -h -b 128 -c ".$utc."/".$file." | dd of=".$dev." bs=64k";
		}
				
    Dada::logMsg(2, $dl, "tarFile: ".$cmd);
    ($result, $response) = Dada::mySystem($cmd);

    # fatal errors, give up straight away
    if ($response =~ m/Input\/output error/) 
    {
      Dada::logMsg(0, $dl, "tarFile: fatal tape error: ".$response);
    ...
    {
      Dada::logMsg(0, $dl, "tarFile: tape unexpectedly ".$tape." full");
      $tar_beam_result = "fail";
    ...
    {
      Dada::logMsg(2, $dl, "tarFile: ".$result." ".$response);
      ...
      if ($tries <= 10)
      {
        Dada::logMsg(1, $dl, "tarFile: failed to write archive to tape, attempt ".(20-$tries)." of 20");
      } 
      sleep(1);
    } 
    ...
    else
    {
      ...
      elsif ( ($est_size_gbytes - $gbytes_written) > 0.01)
      {
        ...
        Dada::logMsg(0, $dl, "tarFile: ".$result." ".$response);
        $tar_beam_result = "fail";
      ...
      {
        $tar_beam_result = "ok";
        Dada::logMsg(2, $dl, "tarFile: est_size ".sprintf("%7.4f GB", $est_size_gbytes).
  ...
  {
    Dada::logMsg(0, $dl, "tarFile: failed to write archive to tape: ".$tar_beam_response);
  ...
  ($result, $filenum, $blocknum) = Dada::tapes::getTapeStatus();
  if ($result ne "ok") {
    Dada::logMsg(0, $dl, "tarFile: getTapeStatus() failed.");
    return ("fail", "getTapeStatus() failed");
  }
  
  if (($blocknum ne 0) || ($filenum ne ($nfiles+1))) {
    Dada::logMsg(0, $dl, "tarFile: positioning error after write: filenum=".$filenum.", blocknum=".$blocknum);
  ...
  Dada::logMsg(3, $dl, "tarFile: updatesTapesDB($id, $size, $used, $free, $nfiles, $full)");
  ($result, $response) = updateTapesDB($id, $size, $used, $free, $nfiles, $full);
  Dada::logMsg(3, $dl, "tarFile: updatesTapesDB(): ".$result." ".$response);
  if ($result ne "ok") {
    Dada::logMsg(0, $dl, "tarFile: updateTapesDB() failed: ".$response);
    return("fail", "error ocurred when updating tapes DB: ".$response);
  }

  Dada::logMsg(3, $dl, "tarFile: updatesFilesDB(".$utc."/".$file.", ".$id.", ".$gbytes_written.", ".($nfiles-1).")");
  ($result, $response) = updateFilesDB($utc."/".$file, $id, $gbytes_written, ($nfiles-1));
  Dada::logMsg(3, $dl, "tarFile: updatesFilesDB(): ".$result." ".$response);
  if ($result ne "ok") {
    Dada::logMsg(0, $dl, "tarFile: updateFilesDB() failed: ".$response);
...
#
sub moveCompletedFile($$$$) 
{
  my ($dir, $utc, $file, $dest) = @_;
  Dada::logMsg(3, $dl, "moveCompletedFile(".$dir.", ".$utc.", ".$file.", ".$dest.")");
  ...
  if (! -d $dir."/".$dest."/".$utc)
  {
    $cmd = "mkdir -p ".$dir."/".$dest."/".$utc;
    Dada::logMsg(3, $dl, "moveCompletedFile: ".$cmd);
    ($result, $response) = Dada::mySystem($cmd);
    Dada::logMsg(3, $dl, "moveCompletedFile: ".$result." ".$response);
    if ($result ne "ok")
    {
      Dada::logMsg(0, $dl, "moveCompletedFile: failed to create ".$dest."/".$utc.": ".$response);
      return ("fail", "failed to create ".$dest."/".$utc);
    }
  }

  # move the file link to the dest dir
  $cmd = "mv ".$dir."/archive/".$utc."/".$file." ".$dir."/".$dest."/".$utc."/";
  Dada::logMsg(3, $dl, "moveCompletedFile: ".$cmd);
  ($result, $response) = Dada::mySystem($cmd);
  Dada::logMsg(3, $dl, "moveCompletedFile: ".$result." ".$response);
  ...
  if ($result ne "ok") 
  {
    Dada::logMsg(0, $dl, "moveCompletedFile: failed to move ".$utc."/".$file." to ".$dest." dir: ".$response);
    return ("fail", "failed to move dir");
  } 
  else 
  {
    # if there are no other files in the directory, then we can delete it
    $cmd = "find -L ".$dir."/archive/".$utc."/ -mindepth 1 -type f | wc -l";
    Dada::logMsg(2, $dl, "moveCompletedFile: ".$cmd);
    ($result, $response) = Dada::mySystem($cmd);
    Dada::logMsg(3, $dl, "moveCompletedFile: ".$result." ".$response);
    ...
    if (($result eq "ok") && ($response eq "0"))
    {
      ...
      if ($result ne "ok") 
      {
        Dada::logMsg(0, $dl, "moveCompletedFile: could not delete ".$dir."/archive/".$utc.": ".$response);
      }
    }
  }
