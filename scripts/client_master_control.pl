...
use constant PIDFILE          => "master_control.pid";
use constant LOGFILE          => "master_control.log";
...
while (!$quit) {
  ...
  logMessage(2, "command: ".$command);

  ($result, $response) = handleCommand($command);

  logMessage(2, "result: ".$result.", response: ".$response);
...
sub handleCommand($) {
  ...
  if ($commands[0] eq "clean_scratch") {
    ...
    #$cmd = $current_binary_dir."/".$cfg{"PWC_BINARY"}." -d -i 192.168.1.117 -p ".$cfg{"CLIENT_UDPDB_PORT"};
    $cmd = $current_binary_dir."/".$cfg{"PWC_BINARY"}." -d -p ".$cfg{"CLIENT_UDPDB_PORT"};
    ($result,$response) = mysystem($cmd, 0);
  }
  ...
  elsif ($commands[0] eq "bpsr_gain") {
    $cmd = $current_binary_dir."/client_ibob_level_setter.pl ".$commands[1];
    ($result,$response) = mysystem($cmd, 0);
  }
