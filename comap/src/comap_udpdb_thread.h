...
#ifndef __COMAP_UDPDB_THREAD_H
#define __COMAP_UDPDB_THREAD_H
...
typedef struct {
  ...
  comap_sock_t *     sock;               // UDP socket for data capture
...
time_t comap_dpdb_start (udpdb_t * ctx, char * header);

// main workhorse function to receive data for a single observation
void * comap_udpdb_receive_obs (void * ctx);
