...
class machine_summary extends apsr_webpage 
{
  // Datablock IDs
  var $db_ids = array();
...
?>
      ...
      function handle_machine_summary_request_old(ms_http_request) 
      {
        if (ms_http_request.readyState == 4) {
          ...
          if (response.length > 0) {
            ...
            //alert("response.length > 0), indexOf="+response.indexOf("Could not connect to"));
              ...
              for (i=0; i < lines.length; i++) {
                ...
                if (machine_update_line) {
                  ...
                  if ((state == "stopped") || (bits.length != 5)) {
                    ...
                    disks_space = disks_space.toFixed(1);
...
<?
    ...
    foreach ($this->machines as $m => $type)
    {
      ...
      for ($i=0; $i<count($m_pwcs); $i++)
      {
        ...
        echo "     <td class='gap' width='40px' style='vertical-align: middle;'>\n";
        if (strpos($mp, "server") === FALSE)
        {
          $tag = $m."_".$mp."_".$db_id;
