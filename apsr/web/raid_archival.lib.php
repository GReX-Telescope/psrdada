...
class raid_archival extends apsr_webpage
{
  var $inst = 0;
  var $cfg = array();
  var $nobs = 0;
  ...
  function raid_archival()
  {
    ...
    $this->title = "APSR | RAID Archival Pipeline";
      ...
      function handle_network_rates_request(nr_http_request) 
      {
        if (nr_http_request.readyState == 4) {
          ...
          for (i=0; i<lines.length; i++)
          {
            ...
            if (parts[0] == "APSR") 
            {
              document.getElementById("apsr_rate").innerHTML = parts[1] + " MB/s";
            }
          }
        }
      }
...
<?
  ...
  function printHTML()
  {
    $this->openBlockHeader("APSR RAID Archival Pipeline");
?>
    <table width='100%' border='0'>
      <tr>
        ...
        <th>To Swin</th>
        <th>Waiting for<br/>PSRFITS Patching</th>
        <th>Patching</th>
...
<?
  ...
  function printUpdateHTML($get)
  {
    ...
    if ($get["type"] == "disk_usage") 
    {
      ...
      $cmd = "cd /lfs/raid0/apsr/; du -sb finished/P* swin/send/P* | awk -F/ '{print $1\" \"$(NF)}' | awk '{print $3\" \"$1}'";
      exec($cmd, $lines);
      ...
      $lines = array();
      $results = array();
      $cmd = "cd /lfs/raid0/apsr; du -sb swin/sent/P* psrfits/unpatched/P* | awk -F/ '{print $1\" \"$(NF)}' | awk '{print $3\" \"$1}'";
      exec($cmd, $lines);
      for ($i=0; $i<count($lines); $i++)
      {
        list ($pid, $size) = split(" ", $lines[$i]);
      ...
      $lines = array();
      $results = array();
      $cmd = "cd /lfs/raid0/apsr; du -sb atnf/send/P* | awk -F/ '{print $1\" \"$(NF)}' | awk '{print $3\" \"$1}'";
      exec($cmd, $lines);
      for ($i=0; $i<count($lines); $i++) 
      {
        list ($pid, $size) = split(" ", $lines[$i]);
      ...
      $lines = array();
      $results = array();
      $cmd = "cd /lfs/raid0/apsr; du -sb atnf/sent/P* | awk -F/ '{print $1\" \"$(NF)}' | awk '{print $3\" \"$1}'";
      exec($cmd, $lines);
      for ($i=0; $i<count($lines); $i++)
      {
        list ($pid, $size) = split(" ", $lines[$i]);
