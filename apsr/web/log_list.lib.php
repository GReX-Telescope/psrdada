...
class log_list extends apsr_webpage 
{
  ...
  function log_list()
  {
    ...
    //array_push($this->css, "/apsr/buttons.css");
...
<?
          for ($i=0; $i<count($this->server_daemons); $i++) {
            if ($this->server_log_info[$this->server_daemons[$i]]["type"] == "server") {
...
?>
    ...
    <span>
      Scroll: 
      <input type="checkbox" id="auto_scroll" onchange="autoScroll()">
    </span>&nbsp;&nbsp;

    <!-- Filter Logs -->
    <span>
      Filter: <input type="text" id="filter" size='8' onchange="changeLogWindow()">
    </span>&nbsp;&nbsp;
