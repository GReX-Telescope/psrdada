...
/* Stop all the server daemons */
  ...
  for ($i=0; $i<count($server_daemons); $i++) {
    if (file_exists($control_dir."/".$server_daemons[$i].".quit")) {
  ...
  /* the daemon existed, launch it! */
    ...
    sleep(2);
    $return_val = waitForDaemon($server_names_combined[$i], $server_daemons_combined[$i], $control_dir);
    if (file_exists($control_dir."/".$server_daemons_combined[$i].".quit"))
