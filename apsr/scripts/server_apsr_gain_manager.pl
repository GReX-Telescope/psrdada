...
my $reporter_socket = "";   # Server socket for new connections
my $rh = "";
my $string = "";

# Setup the gains array
my @pol0_gains = ();
my @pol1_gains = ();
...
$reporter_socket = new IO::Socket::INET (
    LocalHost => $cfg{"SERVER_HOST"},
    LocalPort => $cfg{"SERVER_GAIN_REPORT_PORT"},
    Proto => 'tcp',
    Listen => 1,
    Reuse => 1,
);
...
$read_set->add($reporter_socket); # add the reporter socket to the set
