...
$tcs_cfg_file = $cfg{"CONFIG_DIR"}."/apsr_tcs.cfg";
$tcs_spec_file = $cfg{"CONFIG_DIR"}."/apsr_tcs.spec";
$warn = ""; 
$error = ""; 
$quit_daemon = 0;
$tcs_host = "";
...

{
  ...
  while (! $quit_daemon) {
    ...
    if (($rec_start_unixtime > 0) && ($current_state =~ m/^Rec/))
    {
      $current_state = "Recording [".(time - $rec_start_unixtime)." s]";
    }
...
sub processTCSCommand($$) {
  ...
  switch ($lckey) {
    ...
    case "bat" {
      ...
      if ((!($current_state =~ m/^Rec/)) && ($current_state ne "Starting"))  {
        Dada::logMsgWarn($error, "Received BAT command, but not in RECORDING or STARTING state");
        ...
        if ($result ne "ok") {
          ...
          # used for display length of observation recorded
          $rec_start_unixtime = getUnixTimeUTC($utc_start);
    ...
    case "set_utc_start" {
      ...
      if (!($current_state =~ m/^Rec/)) {
        ...
        if ($use_dfb_simulator) {
          ...
          Dada::logMsg(1, $dl, "processTCSCommand: startDFB(".$val.")");
          ($result, $response) = startDFB($val);
          Dada::logMsg(1, $dl, "processTCSCommand:: startDFB() ".$result." ".$response);
        ...
        if ($result eq "ok") {
          ...
          $rec_start_unixtime = getUnixTimeUTC($val);
    ...
    # This should be a header parameter, add it to the tcs_cmds hash
      ...
      if (($current_state eq "Idle") || ($current_state eq "Preparing")) {
        ...
        $result = "fail";
        Dada::logMsgWarn($warn, "Received header from TCS when not in IDLE state [".$current_state."]"); 
        $current_state = "Error";
      }
    }
  }
...
sub startThread() 
{
  ...
  if (!$pwcc_running) {
    ...
    Dada::logMsg(0, $dl, "startThread: pwcc_thread still did not start");
    $result = "fail";
  ...
  if ($result eq "fail") {
    $current_state = "Failed to start. Must Restart APSR";
  } else {
...
sub stateThread() {
  ...
  Dada::logMsg(1, $dl, "stateThread: exiting");
}
...
sub quitPWCCommand() {
  ...
  if (! $pwcc_running) {
    ...
    # try to kill the process manually
      ...
      Dada::logMsg(1, $dl, "quitPWCCommand: killProcess(^dada_pwc_command.*".$tcs_cfg_file.")");
      ($result, $response) = Dada::killProcess("^dada_pwc_command.*".$tcs_cfg_file);
      Dada::logMsg(1, $dl, "quitPWCCommand: killProcess() ".$result." ".$response); 
...
sub prepareObservation($) {
  ...
  # ensure we are still connected to the nexus
  nexusSockMaintain();
...
sub createDFBSimulator() {
  ...
  my $dfb_cmd = "cmd=dfbsimulator&args=-d -v -a ".$args;
...
sub startDFB($) {
  ...
  Dada::logMsg(1, $dl, "startDFB(".$utc_start.")");
