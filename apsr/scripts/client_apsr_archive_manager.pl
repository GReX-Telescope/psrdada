...
use constant  PIDFILE     => "apsr_archive_manager.pid";
use constant  LOGFILE     => "apsr_archive_manager.log";
...
while (!($quit_daemon)) {
  ...
  $cmd = "find . -name \"20*.ar\" | sort";
  $find_result = `$cmd`;

  @lines = split(/\n/,$find_result);
  foreach $line (@lines) {
    $line = substr($line,2);
    logMessage(1, "INFO", "Processing archive \"".$line."\"");
    processArchive($line);
  }
...
sub processArchive($) {
  ...
  my $cmd = "find ".$file." -type f -name \"*.ar\" -printf \"%h\\n\"";
  logMessage(2, "INFO", $cmd);
  my $remote_dir = `$cmd`;
  chomp $remote_dir;

  my $pamfile = pamTheArchive($file);

  sendToServerViaNFS($pamfile, $cfg{"SERVER_RESULTS_NFS_MNT"}, $remote_dir);
  unlink($pamfile);
...
sub logMessage($$$) {
  ...
  if ($level <= DEBUG_LEVEL) {
    ...
    if ($log_socket) {
      Dada->nexusLogMessage($log_socket, $time, "sys", $type, "arch mngr", $message);
    }
    print "[".$time."] ".$message."\n";
  }
}
...
sub pamTheArchive($) {
  ...
  # PAM it!
  logMessage(2, "INFO", "Pamming file \"".$file."\"");

  $cmd = $bindir."/pam -f8 -b4 -e lowres ".$file;
  ($result, $response) = Dada->mySystem($cmd);

  if ($result ne "ok") {
    logMessage(0, "WARN", "Failed to shrink archive with pam \"".$response."\"");
  }
