...
###############################################################################
# 
# Performs a hard stop of the APSR Instrument
#
...
debugMessage(0, "Hard Stopping APSR");

push @server_daemons, "apsr_master_control";
push @client_daemons, "apsr_master_control";
push @helper_daemons, "apsr_master_control";
...
# kill GPU binaries
debugMessage(0, "Destroying PWC Binaries");
$binaries = $cfg{"PWC_BINARY"}." dspsr digimon dada_dbnull dada_header perl";
($result, $response) = destroyBinaries(\@clients, $binaries);

# destroy GPU data blocks
debugMessage(0, "Destroying PWC Datablocks");
...
sub killServerDaemons($) 
{
  ...
  for ($i=0; $i<=$#daemons; $i++)
  {
    if ($daemons[$i] eq "apsr_master_control")
    {  
