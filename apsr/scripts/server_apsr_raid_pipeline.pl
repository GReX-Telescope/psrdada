...
@r_scripts = ("apsr_bands_finished", "apsr_swin_transferrer", "apsr_psrfits_converter", "apsr_psrfits_patcher", "apsr_cleaner");
...
sub checkRemoteScripts($$$) 
{
  ...
  for ($i=0; $i<=$#r_scripts; $i++)
  {
    ...
    $cmd = "pgrep -u apsr -f '^perl.*raid_".$d.".pl'";
...
sub stopRemoteScripts($$$)
{
  ...
  while (!$scripts_stopped)
  {
    ...
    for ($i=0; $i<=$#r_scripts; $i++)
    {
      ...
      $cmd = "pgrep -u apsr -f '^perl.*raid_".$script.".pl'";
