...
if (!$raw) {
  echo "<html>\n";
  $inst->print_head("BPSR Client Command");
}

# Get machines from the dada.cfg file
$machines = array();

$hosts = determineRunningHosts($inst->config, $dests, $host);
...
if (count($hosts) == 0) {
  ...
  if ($cmd == "start_pwcs") {
    $script = "client_bpsr_command.pl start_iface ".$host_string;
    $output = array();
    $output = runPerlScript($script);
  }

  $script = "client_bpsr_command.pl ".$cmd." ".$host_string;
  $output = array();
  $output = runPerlScript($script);

  if ($cmd == "stop_pwcs") {
    $script = "client_bpsr_command.pl stop_iface ".$host_string;
    $output = array();
    $output = runPerlScript($script);
  }
...
function getCommandDestinations($cmd, $name) {
  ...
  if (!array_key_exists($cmd, $dests)) {
    ...
    if (($cmd == "start_daemon") || ($cmd =="stop_daemon")) {
      if ($name == "bpsr_master_control") {
        array_push($result, "help", "srv");
      } 
    }
  }
