...
#$client_daemons_info = getClientLogInformation();
...
function printClientStatus($config, $control) {
  ...
  $client_daemons["bpsr_master_control"] = array("name" => "Master Control");
  ...
  if ($return_var == 0) {
    ...
    for($i=0; $i<count($output); $i++) {
      ...
      if ( (strpos($string, "Could not connect to machine")) !== FALSE) {
        ...
        $results[$machine]["bpsr_master_control"] = 2;
  ...
  echo "<tr>\n";
  echo "  <td align=right style='vertical-align: middle;'>".$client_daemons["bpsr_master_control"]["name"]."</td>\n";
  for ($j=0; $j<$config["NUM_PWC"]; $j++) {
    echo "  <td width=17 style='vertical-align: middle;'>".statusLight($results[$config["PWC_".$j]]["bpsr_master_control"])."</td>\n";
  }
  if ($control) {
    ?>
    <td><div class="btns">
