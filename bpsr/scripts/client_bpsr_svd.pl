...
{
  print STDERR "SVD_ID [".$svd_id."] was not a valid integer between 0 and ".($cfg{"NUM_SVD"}-1)."\n";
  usage();
  exit(1);
}
...
#
{
  ...
  my $in_key  = Dada::getDBKey($cfg{"DATA_BLOCK_PREFIX"}, $svd_id, $cfg{"NUM_SVD"}, $cfg{"DIRTY_DATA_BLOCK"});
  my $out_key  = Dada::getDBKey($cfg{"DATA_BLOCK_PREFIX"}, $svd_id, $cfg{"NUM_SVD"}, $cfg{"CLEAN_DATA_BLOCK"});
	my $curr_raw_header = "";
	my $prev_raw_header = "";
  my %h = ();
  ...
  while (!$quit_daemon) 
  {
    ...
    else
    {
      if ($curr_raw_header eq $prev_raw_header)
        ...
        $proc_cmd = "bpsr_svd ".$in_key." ".$out_key;
      }

      # setup the full processing command
      $full_cmd = $proc_cmd." 2>&1 | ".$cfg{"SCRIPTS_DIR"}."/client_bpsr_src_logger.pl ".$svd_id." svd";

      logMsg(1, "INFO", "START [svd] ".$proc_cmd);
      ($result, $response) = Dada::mySystem($full_cmd);
      logMsg(1, "INFO", "END   [svd] ".$proc_cmd);
      if ($result ne "ok")
      {
...
sub controlThread($) 
{
  ...
  push @processes_to_kill, "^bpsr_svd ".$in_key." ".$out_key;
