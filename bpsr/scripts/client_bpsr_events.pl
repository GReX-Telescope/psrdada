...
#
# Monitor primary datablock for events
#
...
#
{
  my ($cmd, $result, $response, $proc_cmd);
  ...
  while (!$quit_daemon) 
  {
    ...
    $cmd = "dada_header -t event -k ".$recv_db_key;
    logMsg(2, "INFO", "main: ".$cmd);
    ($result, $curr_raw_header) = Dada::mySystem ($cmd);
    logMsg(3, "INFO", "main: ".$curr_raw_header);

    # processing command for dada_dbevent
    $proc_cmd = "dada_dbevent ".$recv_db_key." ".$dump_db_key." -p ".$event_port." -t 96 ";
    if ($result ne "ok")
		{
      if ($quit_daemon)
      {
    ...
    else
    {
      if ($curr_raw_header eq $prev_raw_header)
      {
        ...
        $proc_cmd = "dada_dbnull -k ".$recv_db_key." -s";
      } 
      else
      {
        ...
        if ($h{"MODE"} eq "CAL")
        {
          logMsg(0, "INFO", "main: ignoring CAL observation");
          $proc_cmd = "dada_dbnull -k ".$recv_db_key." -s";
        }
        elsif ($tobs < 30)
        {
          logMsg(0, "INFO", "main: ignoring short [< 30s] observation: ".$tobs."s");
          $proc_cmd = "dada_dbnull -k ".$recv_db_key." -s";
        }
        else
        {
...
sub controlThread($) 
{
  ...
  $process = "^dada_header -t event -k ".$recv_db_key;
  $user = "bpsr";
  ...
  push @processes_to_kill, "^dada_dbevent ".$recv_db_key;
  push @processes_to_kill, "^dada_dbnull -k ".$recv_db_key;
...
sub logMsg($$$) {
  ...
  if ($level <= $dl) {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, $pwc_id, $time, "sys", $type, "evnt", $msg);
    }
