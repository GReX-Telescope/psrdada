...
#
{
  ...
  my $db_key  = Dada::getDBKey($cfg{"DATA_BLOCK_PREFIX"}, $pwc_id, $cfg{"NUM_PWC"}, $cfg{"CLEAN_DATA_BLOCK"});
	my $curr_raw_header = "";
	my $prev_raw_header = "";
  my %h = ();
    ...
    else
    {
      if ($curr_raw_header eq $prev_raw_header)
        ...
        else
        {
          ...
          $proc_cmd = "bpsr_nicdb -k ".$db_key." ".$pwc_id." ".$ct_file;
        }
      }

      logMsg(1, "INFO", "START [clean_recv] ".$proc_cmd);
      ($result, $response) = Dada::mySystemPiped($proc_cmd, $src_log_file, $src_log_sock,
                                               "src", sprintf("%02d",$pwc_id),
                                               $daemon_name, "clean_recv");
      logMsg(1, "INFO", "END   [clean_recv] ".$proc_cmd);
      if ($result ne "ok")
      {
...
sub logMsg($$$) {
  ...
  if ($level <= $dl) {
    ...
    if ($sys_log_sock) {
      Dada::nexusLogMessage($sys_log_sock, $pwc_id, $time, "sys", $type, "scrunch", $msg);
    }
