...
sub setupClientType() 
{
  ...
  my $i = 0;
  my $found = 0;
  my %ct = Bpsr::getCornerturnConfig();
  ...
  $Dada::client_master_control::num_pwc = $cfg{"NUM_SVD"};
  ...
  for ($i=0; ($i<$cfg{"NUM_SVD"}); $i++)
  {
    if ($host =~ m/$cfg{"SVD_".$i}/)
    {
  ...
  # see how many SVDs are running on this host
  for ($i=0; ($i<$cfg{"NUM_SVD"}); $i++) 
  {
    if ($host =~ m/$cfg{"SVD_".$i}/) 
    {
      # add to list of SVDs on this host
      $Dada::client_master_control::pwcs{$i} = ();

      # hash for the data blocks for this SVD
      ...
      foreach $id (@ids) 
      {
        $key = Dada::getDBKey($cfg{"DATA_BLOCK_PREFIX"}, $i, $cfg{"NUM_SVD"}, $id);
