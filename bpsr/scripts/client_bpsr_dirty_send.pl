...
#
{
  ...
  my $db_key  = Dada::getDBKey($cfg{"DATA_BLOCK_PREFIX"}, $pwc_id, $cfg{"NUM_PWC"}, $cfg{"DIRTY_DATA_BLOCK"});
	my $curr_raw_header = "";
	my $prev_raw_header = "";
  my %h = ();
    ...
    else
    {
      if ($curr_raw_header eq $prev_raw_header)
        ...
        else
        {
          ...
          $proc_cmd = "bpsr_dbnic -k ".$db_key." ".$pwc_id." ".$ct_file;
        }
      }

      # setup the full processing command
      $full_cmd = $proc_cmd." 2>&1 | ".$cfg{"SCRIPTS_DIR"}."/client_bpsr_src_logger.pl ".$pwc_id." dirty_send";

      logMsg(1, "INFO", "START [dirty_send] ".$proc_cmd);
      ($result, $response) = Dada::mySystem($full_cmd);
      logMsg(1, "INFO", "END   [dirty_clean] ".$proc_cmd);
      if ($result ne "ok")
      {
...
sub logMsg($$$) {
  ...
  if ($level <= $dl) {
    ...
    if ($log_sock) {
      Dada::nexusLogMessage($log_sock, $pwc_id, $time, "sys", $type, "scrunch", $msg);
    }
