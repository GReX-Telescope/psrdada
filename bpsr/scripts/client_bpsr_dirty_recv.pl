...
if (($svd_id >= 0) &&  ($svd_id < $cfg{"NUM_SVD"}))
{
  ...
  {
    # determine the relevant SVD based configuration for this script 
    $beam = $roach{"BEAM_".$svd_id};
  }
  else
  {
...
#
{
  ...
  my $db_key  = Dada::getDBKey($cfg{"DATA_BLOCK_PREFIX"}, $svd_id, $cfg{"NUM_SVD"}, $cfg{"DIRTY_DATA_BLOCK"});
	my $curr_raw_header = "";
	my $prev_raw_header = "";
  my %h = ();
    ...
    else
    {
      if ($curr_raw_header eq $prev_raw_header)
        ...
        else
        {
          ...
          $proc_cmd = "bpsr_nicdb -k ".$db_key." ".$svd_id." ".$ct_file;
        }
      }

      # setup the full processing command
      $full_cmd = $proc_cmd." 2>&1 | ".$cfg{"SCRIPTS_DIR"}."/client_bpsr_src_logger.pl ".$svd_id." dirty_recv";

      logMsg(1, "INFO", "START [dirty_recv] ".$proc_cmd);
      ($result, $response) = Dada::mySystem($full_cmd);
      logMsg(1, "INFO", "END   [dirty_recv] ".$proc_cmd);
      if ($result ne "ok")
      {
...
sub controlThread($) 
{
  ...
  push @processes_to_kill, "^bpsr_nicdb -k ".$db_key;
