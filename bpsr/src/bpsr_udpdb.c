...
void* udpdb_buffer_function (dada_pwc_main_t* pwcm, uint64_t* size)
{
  ...
  uint64_t buffer_capacity = udpdb->datasize / BPSR_UDP_DATASIZE_BYTES;
  ...
  *size = buffer_capacity * BPSR_UDP_DATASIZE_BYTES;
  ...
  while (!quit) {
    ...
    if (!quit) {
      ...
      sequence_no = decode_header(udpdb->socket_buffer);
      ...
      if ((udpdb->expected_sequence_no == 0) && (data_received == 0)) {
        ...
        if (sequence_no != 0) {
          ...
          if ((sequence_no * udpdb->received) > (1024*1024*1024)) {
        ...
        if (udpdb->received != BPSR_UDP_PAYLOAD_BYTES) {
          multilog (log, LOG_ERR, "UDP packet size was incorrect\n");
          *size = DADA_ERROR_HARD;
          break;
        }
      ...
      udpdb->bytes_received_this_run += BPSR_UDP_DATASIZE_BYTES;
      udpdb->bytes_received += BPSR_UDP_DATASIZE_BYTES;
 
      /* If the packet belongs in the curr_buffer */
      if ((sequence_no >= min_sequence) && 
          (sequence_no <  mid_sequence)) {

        uint64_t buf_offset = (sequence_no - min_sequence) * BPSR_UDP_DATASIZE_BYTES;
        ...
        uint64_t buf_offset = (sequence_no - mid_sequence) * BPSR_UDP_DATASIZE_BYTES;
  ...
  if (data_received) {
    ...
    if ((udpdb->curr_buffer_count < buffer_capacity) && (!timeout_ocurred)) {
      ...
      udpdb->packets_dropped_this_run += buffer_capacity - udpdb->curr_buffer_count;
    }
  ...
  if (n_dropped * BPSR_UDP_DATASIZE_BYTES > drop_threshold) {
    multilog (log, LOG_ERR, "Too many udp packets dropped. Threshold is"
              " %"PRIu64" bytes , but dropped %"PRIu64" bytes \n",
              drop_threshold, n_dropped*BPSR_UDP_DATASIZE_BYTES); 
    *size = DADA_ERROR_HARD;
  }*/
