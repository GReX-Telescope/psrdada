...
{
  document.clock.longitude.value = longitude;
...
function loadUserSetting(longitude)
{
  ...
  if (document.clock.longitude.value == "null")
  {
    document.clock.longitude.value = 0;   // no saved data so load form input field with longitude for GMST
  }
...
{
  var long = parseFloat (document.clock.longitude.value);   // get longitude variable from current form INPUT text value
...
function UpdateSimpleClocks()   //loop to keep time displays current 
{
  ...
  document.getElementById("date").innerHTML = " " + date;  // update "clock" form displays
