...
int main (int argc, char **argv)
{
  ...
  while ((arg=getopt(argc,argv,"b:i:p:s:vh")) != -1) 
  {
    switch (arg) 
    {
      case 'b': 
  ...
  multilog_t * log = multilog_open ("ska1_udptest", 0);
  ...
  // UDP file descriptor
  multilog(log, LOG_INFO, "creating listening socket on %s:%d\n", interface, inc_port);
  ...
  // set the socket size to 1 MB
  int sock_buf_size = 1048576;
  multilog(log, LOG_INFO, "setting buffer size to %d\n", sock_buf_size);
  dada_udp_sock_set_buffer_size (log, fd, verbose, sock_buf_size);

  // set the socket to non-blocking
  multilog(log, LOG_INFO, "setting non_block\n");
  sock_nonblock(fd);

  // clear any packets buffered by the kernel
  multilog(log, LOG_INFO, "clearing packets at socket\n");
  size_t cleared = dada_sock_clear_buffered_packets (fd, pkt_size);

  // set the socket to non-blocking
  //multilog(log, LOG_INFO, "setting block\n");
  //sock_block(fd);
...
void stats_thread(void * arg) {
  ...
  ska1_udptest_t * ctx = (ska1_udptest_t *) arg;
