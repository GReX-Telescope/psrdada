 ...
 * ska1_udpdb_thread
 *
 * Reads UDP packets for SKA1 correlator, uses:
 ...
 */
int ska1_udpdb_init_receiver (udpdb_t * ctx)
{
  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "ska1_udpdb_init_receiver()\n");

  // create a SKA1 socket which can hold variable num of UDP packet
  ctx->sock = ska1_init_sock();
 ...
 */
int ska1_udpdb_destroy_receiver (udpdb_t * ctx)
{
  if (ctx->sock)
    ska1_free_sock(ctx->sock);
  ctx->sock = 0;
}
 ...
 */
void ska1_udpdb_reset_receiver (udpdb_t * ctx) 
{
  if (ctx->verbose)
    multilog (ctx->log, LOG_INFO, "ska1_udpdb_reset_receiver()\n");
...
int ska1_udpdb_prepare (udpdb_t * ctx)
{
  if (ctx->verbose)
    multilog(ctx->log, LOG_INFO, "ska1_udpdb_prepare()\n");
 ...
 */
time_t ska1_udpdb_start (udpdb_t * ctx, char * obs_header)
{
  ...
  // read the BYTES_PER_SECOND to determine stopping byt
  if (ctx->secs_to_acquire > 0)
  {
    uint64_t bytes_per_second;
    if (ascii_header_get (header, "BYTES_PER_SECOND", "%"PRIu64, &bytes_per_second) < 0)
    {
      multilog (ctx->log, LOG_ERR, "Could not read BYTES_PER_SECOND from header\n");
      return -1;
    }
    stop_byte = (uint64_t) ctx->secs_to_acquire * bytes_per_second;
    //if (ctx->verbose)
      multilog (ctx->log, LOG_INFO, "start: setting stop byte to %"PRIu64"\n", stop_byte);
  }
  ...
  if (ska1_udpdb_open_buffer (ctx) < 0)
  {
    multilog (ctx->log, LOG_ERR, "start: ska1_udpdb_open_buffer failed\n");
    return -1;
  }
 ...
 */
int ska1_udpdb_open_buffer (udpdb_t * ctx)
{

  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "ska1_udpdb_open_buffer()\n");
 ...
 */
int ska1_udpdb_close_buffer (udpdb_t * ctx, uint64_t bytes_written, unsigned eod)
{

  if (ctx->verbose && stop_pending || ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "ska1_udpdb_close_buffer(%"PRIu64", %d)\n", bytes_written, eod);
 ...
 */
int ska1_udpdb_new_buffer (udpdb_t * ctx)
{

  if (ctx->verbose > 1)
    multilog (ctx->log, LOG_INFO, "ska1_udpdb_new_buffer()\n");

  if (ska1_udpdb_close_buffer (ctx, ctx->hdu_bufsz, 0) < 0)
  {
    multilog (ctx->log, LOG_INFO, "new_buffer: ska1_udpdb_close_buffer failed\n");
    return -1;
  }

  if (ska1_udpdb_open_buffer (ctx) < 0) 
  {
    multilog (ctx->log, LOG_INFO, "new_buffer: ska1_udpdb_open_buffer failed\n");
    return -1;
  }
 ...
 * datablocks
 */
void * ska1_udpdb_receive_obs (void * arg)
{
  ...
  if (ctx->verbose)
    multilog(log, LOG_INFO, "ska1_udpdb_receive_obs()\n");
  ...
  while (!quit_threads && !stop_pending) 
  {
    ...
    if (ctx->sock->have_packet) 
    {
      ...
      //ska1_decode_header(ctx->sock->buf, &seq_no, &ant_id);
      ...
      if (ctx->capture_started)
      {
        ...
        if (ctx->verbose > 2)
          multilog(ctx->log, LOG_INFO, "seq_byte=%"PRIu64" seq_no=%"PRIu64", "
                   "ant_id =%d, UDP_DATA=%d\n",seq_byte, seq_no,ant_id, UDP_DATA);
      ...
      if ((ctx->block_count >= ctx->packets_per_buffer) || (temp_idx >= temp_max))
      {
        ...
        if (ska1_udpdb_new_buffer (ctx) < 0)
        {
          multilog(ctx->log, LOG_ERR, "receive_obs: ska1_udpdb_new_buffer failed\n");
          thread_result = -1;
          pthread_exit((void *) &thread_result);
        }
    ...
    if (stop_byte)
    {
      if (seq_byte >= stop_byte)
      {
        ...
        if (ska1_udpdb_close_buffer (ctx, bytes_just_written, 1) < 0)
        {
          multilog(ctx->log, LOG_ERR, "receive_obs: ska1_udpdb_close_hdu failed\n");
          thread_result = -1;
          pthread_exit((void *) &thread_result);
        }
        stop_byte = 0;
...
int main (int argc, char **argv)
{
  ...
  int inc_port = SKA1_DEFAULT_UDPDB_PORT;

  /* multilog output port */
  int l_port = SKA1_DEFAULT_PWC_LOGPORT;
  ...
  while ((arg=getopt(argc,argv,"b:c:df:i:k:l:p:t:vh")) != -1) {
    switch (arg) {
  ...
  log = multilog_open ("ska1_udpdb_thread", 0);
  ...
  multilog (log, LOG_INFO, "main: ska1_udpdb_init_receiver()\n");
  if (ska1_udpdb_init_receiver (&udpdb) < 0)
  {
  ...
  {
    if (verbose)
      multilog(log, LOG_INFO, "main: ska1_udpdb_reset_receiver()\n");
    ska1_udpdb_reset_receiver (&udpdb);
    ...
    udpdb.secs_to_acquire = (int64_t) nsecs;

    // if header was supplied via text file, begin immediately
    if (header_file)
    {
      if (verbose)
        multilog(log, LOG_INFO, "main: ska1_udpdb_start()\n");
      time_t utc = ska1_udpdb_start (&udpdb, obs_header);
      if (utc == -1 ) {
    ...
    rval = pthread_create (&receiving_thread_id, 0, (void *) ska1_udpdb_receive_obs , (void *) &udpdb);
    if (rval != 0) {
      multilog(log, LOG_INFO, "Error creating ska1_udpdb_receive_obs thread: %s\n", strerror(rval));
      return -1;
    }

    if (verbose) 
      multilog(log, LOG_INFO, "joining ska1_udpdb_receive_obs thread\n");
    pthread_join (receiving_thread_id, &result);
    multilog(log, LOG_INFO, "joined ska1_udpdb_receive_obs thread\n");
  ...
  // clean up memory 
  if ( ska1_udpdb_destroy_receiver (&udpdb) < 0) 
