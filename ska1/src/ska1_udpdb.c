...
typedef struct {
  ...
  ska1_sock_t * sock;             // custom socket struct
...
void usage()
{
  fprintf (stdout,
     ...
     DADA_DEFAULT_BLOCK_KEY, SKA1_DEFAULT_UDPDB_PORT);
}

int ska1_init (ska1_udpdb_t * ctx)
{
...
int ska1_udpdb_open (dada_client_t* client)
{
  assert (client != 0);

  // contextual data for dada_udpdb
  ska1_udpdb_t * ctx = (ska1_udpdb_t *) client->context;
  assert(ctx != 0);

  fprintf(stderr, "ctx->verbose=%d\n", ctx->verbose);
  if (ctx->verbose)
    multilog(ctx->log, LOG_INFO, "ska1_udpdb_open()\n");
...
int64_t ska1_udpdb_recv (dada_client_t* client, void* data, uint64_t data_size)
{
...
int64_t ska1_udpdb_recv_block (dada_client_t* client, void* data, uint64_t data_size, uint64_t block_id)
{
  ska1_udpdb_t * ctx = (ska1_udpdb_t *) client->context;
...
int ska1_udpdb_close (dada_client_t* client, uint64_t bytes_written)
{
...
int main (int argc, char **argv)
{
  ...
  int port = SKA1_DEFAULT_UDPDB_PORT;
  ...
  client->open_function     = ska1_udpdb_open;
  client->io_function       = ska1_udpdb_recv;
  client->io_block_function = ska1_udpdb_recv_block;
  client->close_function    = ska1_udpdb_close;
...
void stats_thread(void * arg) 
{
  ska1_udpdb_t * ctx = (ska1_udpdb_t *) arg;
