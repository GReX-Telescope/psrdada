...
sub configureMultibobServer() {
  ...
  my %cfg   = Hispec::getConfig();
  my %ibobs = Hispec::getIBOBConfig();
  my %roaches  = Hispec::getROACHConfig();
  ...
  ($result, $response) = Hispec::waitForMultibobState("closed", $handle, 10);
  if ($result ne "ok") {
  ...
  ($result, $response) = Hispec::waitForMultibobState("alive", $handle, 60);
  if ($result ne "ok") {
...
sub getConfig() 
{
  my $config_file = $DADA_ROOT."/share/hispec.cfg";
