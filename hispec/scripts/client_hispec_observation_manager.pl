...
# Include Modules
#
use Hispec;          # DADA Module for configuration options
...
sub processObs($$$)
{
  ...
  { 
    ...
    {
      my $tmp_info_file =  "/tmp/hispec_".$db_key.".info";
  ...
  $cmd = "cd ".$processing_dir."; ".$proc_cmd." 2>&1 | ".$cfg{"SCRIPTS_DIR"}."/client_hispec_src_logger.pl ".$pwc_id." proc";
...
sub nullThread($$)
{
  ...
  my $full_cmd = $cmd." 2>&1 | ".$cfg{"SCRIPTS_DIR"}."/client_hispec_src_logger.pl ".$pwc_id." ",$tag;
...
sub controlThread($) 
{
  ...
  $cmd = "ps aux | grep -v grep | grep hispec | grep 'dada_header -k ".$db_key."' | awk '{print \$2}'";
