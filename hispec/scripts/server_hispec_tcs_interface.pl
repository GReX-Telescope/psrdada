...
# Constants
#
use constant PIDFILE            => "hispec_tcs_interface.pid";
use constant LOGFILE            => "hispec_tcs_interface.log";
use constant QUITFILE           => "hispec_tcs_interface.quit";
...
#
{
  ...
  my $pwcc_file = $config_dir."/hispec_tcs.cfg";
  my $utc_start = "";

  # Create the initial hispec_tcs.cfg file to launch dada_pwc_command
  ($result, $response) = generateConfigFile($cfg{"CONFIG_DIR"}."/hispec_tcs.cfg", \%site_cfg);
        ...
        # Else we have received a command
          ...
          if ($lckey eq "start") 
          {
            ...
            if ($result ne "ok") {
              ...
              Dada::logMsg(2, $dl, "main: generateConfigFile(".$cfg{"CONFIG_DIR"}."/hispec_tcs.cfg)");
              ($result, $response) = generateConfigFile($cfg{"CONFIG_DIR"}."/hispec_tcs.cfg", \%tcs_cmds);
              ...
              $pwcc_thread = threads->new(\&pwcc_thread, $cfg{"CONFIG_DIR"}."/hispec_tcs.cfg");

              # Create the hispec_tcs.spec file to launch dada_pwc_command
              Dada::logMsg(2, $dl, "main: generateSpecificationFile(".$cfg{"CONFIG_DIR"}."/hispec_tcs.spec)");
              ($result, $response) = generateSpecificationFile($cfg{"CONFIG_DIR"}."/hispec_tcs.spec", \%tcs_cmds);
              Dada::logMsg(2, $dl, "main: generateSpecificationFile() ".$result." ".$response);
  
              # Issue the start command itself
              Dada::logMsg(2, $dl, "main: start(".$cfg{"CONFIG_DIR"}."/hispec_tcs.spec)");
              ($result, $response) = start($cfg{"CONFIG_DIR"}."/hispec_tcs.spec", \%tcs_cmds);
              if ($result ne "ok") {
...
sub quitPWCCommand() 
{
  ...
  if ($pwcc_running) 
  {
    ...
    Dada::logMsg(0, $dl, "quitPWCCommand: killProcess(dada_pwc_command.*hispec_tcs.cfg)");
    ($result, $response) = Dada::killProcess("dada_pwc_command.*hispec_tcs.cfg");
...
sub start($\%)
{
  ...
  else
  {
    ...
    ($result, $response) = Hispec::waitForMultibobState("active", $roach_mngr, 2);
...
#
# wait for HISPEC to stop in the background
#
...
sub resetNexus($)
{
  ...
  for ($i=0; $i<$cfg{"NUM_PWC"}; $i++) {
    ...
    ($result, $response) = Hispec::clientCommand($cmd, $cfg{"PWC_".$i});
  ...
  Dada::logMsg(1, $dl, "PWCC <- ".$cfg{"CONFIG_DIR"}."/hispec_tcs.cfg");
  $pwcc_thread = threads->new(\&pwcc_thread, $cfg{"CONFIG_DIR"}."/hispec_tcs.cfg");
...
sub generateConfigFile($\%) {
  ...
  print FH $string."\n";
  Dada::logMsg(2, $dl, "hispec_tcs.cfg: ".$string);
...
sub generateSpecificationFile($\%) {
  ...
  foreach $line (@sorted) {
    if (!(exists $ignore{$line})) {
      ...
      Dada::logMsg(2, $dl, "hispec_tcs.spec: ".Dada::headerFormat($line, $tcs_cmds{$line}));
    }
  }
...
sub waitForMultibobBoot() 
{
  ...
  ($result, $response) = Hispec::waitForMultibobState("active", $handle, 60);
