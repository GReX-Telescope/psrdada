...
class control extends hispec_webpage 
{
...
?>
    ...
    <script type='text/javascript'>
      ...
      function startHispec() 
      {
        ...
        url = "control.lib.php?action=start&daemon=hispec_master_control&nhosts=1&host_0="+srv_host
        daemon_action_request(url);

        // start the pwc's master control script
        url = "control.lib.php?action=start&daemon=hispec_master_control&nhosts="+pwc_hosts.length+pwc_hosts_str;
        daemon_action_request(url);
...
?>
      ...
      function startHispecStage3()
      {
        ...
        if ((!pwc_ready) && (stage3_wait > 0)) {
          stage3_wait--;
          setTimeout('startHispecStage3()', 1000);
          return 0;
        }
        stage3_wait = 0;
      ...
      function startHispecStage4()
      {
        poll_2sec_count = 0;
        var pwc_daemons = new Array("<?echo $this->inst->config["PWC_BINARY"]?>");
        var pwc_ready = checkMachinesPWCsAndDaemons(pwc_hosts, pwc_daemons, "green_light.png");

        var srv_daemons = new Array("hispec_pwc_monitor", "hispec_src_monitor", "hispec_sys_monitor", "hispec_tcs_interface", "hispec_results_manager", "hispec_roach_manager", "hispec_web_monitor");
        var srv_ready = checkMachinesAndDaemons(srv_hosts, srv_daemons, "green_light.png");

        if ((!(pwc_ready && srv_ready)) && (stage4_wait > 0)) {
          stage4_wait--;
          setTimeout('startHispecStage4()', 1000);
          return 0;
        }
        stage4_wait = 0;
      ...
      function startHispecStage5()
      {
        poll_2sec_count = 0;
        var pwc_daemons = new Array("hispec_observation_manager","hispec_results_monitor","hispec_disk_cleaner","hispec_transfer_raid");
        var pwc_ready = checkMachinesPWCsAndDaemons(pwc_hosts, pwc_daemons, "green_light.png");

        if ((!pwc_ready) && (stage5_wait > 0)) {
          stage5_wait--;
          setTimeout('startHispecStage5()', 1000);
          return 0;
        }
        stage5_wait = 0;
      ...
      function hardstopHispec()
      {
        ...
        url = "control.lib.php?script=hispec_hard_reset.pl";
        popUp(url);
      }
      ...
      function stopHispec()
      {
        ...
        // stop server TCS interface
        url = "control.lib.php?action=stop&daemon=hispec_tcs_interface&nhosts=1&host_0="+srv_host;
        daemon_action_request(url);
      ...
      function stopHispecStage2()
      {
        poll_2sec_count = 0;

        var srv_daemons = new Array("hispec_tcs_interface");
        var pwc_daemons = new Array("hispec_observation_manager","hispec_results_monitor","hispec_disk_cleaner","hispec_transfer_raid");
        ...
        if ((!(srv_ready && pwc_ready)) && (stage2_wait > 0)) {
          stage2_wait--;
          setTimeout('stopHispecStage2()', 1000);
          return 0;
        }
        stage2_wait = 0;
      ...
      function stopHispecStage3()
      {
        ...
        if ((!pwc_ready) && (stage3_wait > 0)) {
          stage3_wait--;
          setTimeout('stopHispecStage3()', 1000);
          return 0;
        }
        stage3_wait = 0;
...
?>
      ...
      function stopHispecStage4()
      {
        poll_2sec_count = 0;

        var srv_daemons = new Array("hispec_results_manager", "hispec_pwc_monitor", "hispec_sys_monitor", "hispec_src_monitor", "hispec_web_monitor", "hispec_roach_manager");
        var srv_ready = checkMachinesAndDaemons(srv_hosts, srv_daemons, "red_light.png");
        ...
        if ((!(srv_ready && pwc_ready)) && (stage4_wait > 0)) {
          stage4_wait--;
          setTimeout('stopHispecStage4()', 1000);
          return 0;
        }
        stage4_wait = 0;

        // stop the pwc's master control script
        url = "control.lib.php?action=stop&daemon=hispec_master_control&nhosts="+pwc_hosts.length+pwc_hosts_str;
        daemon_action_request(url);

        // stop the server's master control script
        url = "control.lib.php?action=stop&daemon=hispec_master_control&nhosts=1&host_0="+srv_host
        daemon_action_request(url);
      ...
      function stopHispecStage5()
      {
        poll_2sec_count = 0;
        var srv_daemons = new Array("hispec_master_control");
        var srv_ready = checkMachinesAndDaemons(srv_hosts, srv_daemons, "red_light.png");

        var pwc_daemons = new Array("hispec_master_control");
        var pwc_ready = checkMachinesAndDaemons(pwc_hosts, pwc_daemons, "red_light.png");

        if ((!(srv_ready && pwc_ready)) && (stage5_wait > 0)) {
          stage5_wait--;
          setTimeout('stopHispecStage5()', 1000);
          return 0;
        }
        stage5_wait = 0;
...
?>
    ...
    <input type='button' value='Start' onClick="startHispec()">
    <input type='button' value='Stop' onClick="stopHispec()">
    <input type='button' value='Hard Stop' onClick="hardstopHispec()">
<?
...
?>
    <p>Instrument Controls can be used to start/stop/ the all required HISPEC daemons</p>
...
<html>
<head>
  <title>HISPEC Controls</title>
  <link rel='shortcut icon' href='/hispec/images/hispec_favico.ico'/>
<?
...
<?
    $this->printServerDaemonControl("hispec_master_control", "Master Control", $host);
...
<?
    ...
    $this->printClientDaemonControl("hispec_master_control", "Master&nbsp;Control", $this->host_list, "daemon&name=".$d);
...
<?
  ...
  function printUpdateXML($get)
  {
    ...
    for ($i=0; $i<count($hosts); $i++) {
      ...
      if ((!array_key_exists($i, $responses)) || ($responses[$i] == ""))
        $xml .= "<host>".$hosts[$i]."</host><hispec_master_control>0</hispec_master_control>";
      else 
        $xml .= $responses[$i];
  ...
  function printActionHTML($get)
  {
    ...
    if ($area == "persist") {
    ...
    } else if (($daemon == "hispec_master_control") && ($action == "start")) {
      ...
      for ($i=0; $i<$nhosts; $i++) {
        if ($area == "srv") {
          $cmd = "ssh -x -l dada ".$hosts[$i]." client_hispec_master_control.pl";
        } else {
          $cmd = "ssh -x -l hispec ".$hosts[$i]." client_hispec_master_control.pl";
        }
