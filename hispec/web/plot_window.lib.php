...
class plot_window extends hispec_webpage 
{
      ...
      function reset_others(excluded) 
      {
        ...
        for (i=0; i< imgs.length; i++) {
          if ((imgs[i].id.indexOf("beam") == 0) && (excluded.indexOf(imgs[i].id) == -1)) {
            imgs[i].src = "/hispec/images/hispec_beam_disabled_240x180.png";
            tmpstr += "id="+imgs[i].id+",";
          }
        }
      }
      ...
      <tr>
        <td rowspan=3 valign="top" align='left'>
          <form name="imageform" class="smalltext">
            <input type="radio" name="imagetype" id="imagetype" value="ac" onClick="plot_window_request()">Auto<br>
            <input type="radio" name="imagetype" id="imagetype" value="cc" onClick="plot_window_request()">Cross<br>
            <input type="radio" name="imagetype" id="imagetype" value="tp" onClick="plot_window_request()">Total Power<br>
          </form>
        </td>
...
<!--
    <a href="/hispec/transient_window.lib.php?single=true" target="_popup">Transients!!!</a>
-->
  </center>
...
<?
  ...
  function printUpdateHTML($get)
  {
    ...
    $xml .= "<url_prefix>/hispec/results/</url_prefix>";
